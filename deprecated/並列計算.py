...
def potentialFoam(latest_time, domains):
    has_p_potentialflow = False
    #                    0123456789
    for d in glob.iglob('processor*' + os.sep):
        try:
            int(d[9:-len(os.sep)])
            p_potentialflow = os.path.join(d, latest_time, 'p_potentialflow')
            if os.path.isfile(p_potentialflow):
                has_p_potentialflow = True
                os.rename(p_potentialflow, os.path.join(d, latest_time, 'p'))
        except:
            pass
    p_potentialflow = os.path.join(latest_time, 'p_potentialflow')
    if os.path.isfile(p_potentialflow):
        os.remove(p_potentialflow)
    ...
    command = ('mpirun -np %d potentialFoam -parallel' % domains +
    ...
    if subprocess.call(command, shell = True) == 0:
        if os.path.isfile(p_bak):
            ...
            with open(os.path.join('system', 'changeDictionaryDict'), 'w') as f:
                ...
                f.write('\n}\n')
            command = ('mpirun -np %d changeDictionary -parallel' % domains + ' -enableFunctionEntries')
            if subprocess.call(command, shell = True) == 0:
            ...
            os.remove(p_bak)
        subprocess.call("reconstructPar -withZero -time '%s'" % latest_time, shell = True)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        with_function_objects = exec_potentialFoam = exec_paraFoam = redivide_region = False
        domains = 1
        i = 1
        while i < len(sys.argv):
            if sys.argv[i] == '-N': # Non-interactive
                ...
                redivide_region = True
                i += 1
                domains = int(sys.argv[i])
            i += 1
    ...
    if os.path.isdir('processor0'):
        if interactive:
            sys.stdout.write('分割をやり直しますか？ (y/n) > ')
            redivide_region = True if raw_input().strip().lower() == 'y' else False
        if redivide_region:
            #                    0123456789
            for d in glob.iglob('processor*' + os.sep):
                try:
    ...
    if not os.path.isdir('processor0'):
        ...
        if interactive:
            while True:
                sys.stdout.write('領域を何個に分割しますか？ (%d個まで) > ' % threads)
                try:
                    domains = int(raw_input().strip())
                    break
                except:
                    pass
        ...
        with open(decomposeParDict, 'w') as f:
            ...
            f.write('roots\t();\n')
        command = 'decomposePar -latestTime'
        if subprocess.call(command, shell = True) != 0:
            print('%sで失敗しました．よく分かる人に相談して下さい．' % command)
            sys.exit(1)
    else:
        domains = DictParser(decomposeParDict).getValueForKey(['numberOfSubdomains'])[0]
        domains = int(domains) if domains.isdigit() else 1
...
#        potentialFoam(latest_time, domains)
