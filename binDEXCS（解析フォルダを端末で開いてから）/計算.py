...
#        ax.legend(loc = 'best') # ax.plotを呼び出した後
    ...
    def set_subplots():
        ...
        if 'Diffusion' in plot_data:
            set_subplot(data_key = 'Diffusion', xlabel = 'iteration', ylabel= 'Diffusion number',
                window_title = 'iteration histories of Diffusion numbers', logscale = True)
...
#                sys.stdout.flush() # リアルタイム反映のため
    ...
    def relax_delta_sign(recent_residuals):
        ...
        res_slope = np.polyfit(np.arange(recent_residuals.shape[0]), np.log10(recent_residuals), 1)[0]
        s = np.heaviside(res_mean - res_crit, 0.0)
        return -s*np.heaviside(res_slope, 0.0) + (1.0 - s)*(
            np.heaviside(res_flat - abs(res_slope), 0.0) - np.heaviside(res_slope - res_flat, 0.0))
...
def change_relaxationFactors_in_fvSolution(param_name, remark, delta = -0.01, lower_limit = 0.3):
    ...
    if comment is not None and comment['value'].startswith(f'// {remark}, '):
        return
