...
boundary_conditions = (
    (
        ...
        f"{src}finiteVolume/fields/fvPatchFields/basic/calculated",
    ),
    (
        "compressible::alphatWallFunction",
        "alphat = mut/Prt（mut: 乱流粘性係数）から壁面乱流温度拡散率を計算する．",
        "Prt 0.85; // 乱流プラントル数\nvalue $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/compressible/turbulentFluidThermoModels/derivedFvPatchFields/wallFunctions/alphatWallFunctions/alphatWallFunction",
    ),
    (
        ...
        + "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/compressible/turbulentFluidThermoModels/derivedFvPatchFields/turbulentTemperatureCoupledBaffleMixed",
    ),
    (
    ...
    (
        ...
        "",
        f"{src}finiteVolume/fields/fvPatchFields/constraint/cyclic",
    ),
    (
        "empty",
        "1次元または2次元解析の時に，計算しない方向に垂直な面であることを示す．",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/constraint/empty",
    ),
    (
        "epsilonWallFunction",
        "epsilonの壁面境界条件",
        "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/epsilonWallFunctions/epsilonWallFunction",
    ),
    (
        ...
        + "kappaMethod fluidThermo; // 境界内側の熱伝導率を指定\n// 流体側の境界ならfluidThermo，個体側の境界ならsolidThermo\n"
        + "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/compressible/turbulentFluidThermoModels/derivedFvPatchFields/externalWallHeatFluxTemperature",
    ),
    (
        "fixedFluxPressure",
        "速度境界条件を満足するようにp_rghを設定",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/derived/fixedFluxPressure",
    ),
    (
        "fixedGradient",
        "こう配をgradientで決めた値にする．",
        "gradient uniform 1.2; // ベクトルの場合は(1.2 3.4 5.6)のように書く．",
        f"{src}finiteVolume/fields/fvPatchFields/basic/fixedGradient",
    ),
    (
        "fixedValue",
        "valueで決めた値に固定",
        "value uniform 1.2; // ベクトルの場合は(1.2 3.4 5.6)のように書く．",
        f"{src}finiteVolume/fields/fvPatchFields/basic/fixedValue",
    ),
    (
        ...
        + "value $internalField; // 実際には使わないけど必要",
        f"{src}finiteVolume/fields/fvPatchFields/derived/flowRateInletVelocity",
    ),
    (
        "freestreamPressure",
        ...
        f"{src}finiteVolume/fields/fvPatchFields/derived/freestreamPressure",
    ),
    (
        "freestreamVelocity",
        ...
        f"{src}finiteVolume/fields/fvPatchFields/derived/freestreamVelocity",
    ),
    (
        ...
        f"{src}thermophysicalModels/radiation/derivedFvPatchFields/greyDiffusiveViewFactor",
    ),
    (
        "inletOutlet",
        "計算領域内に流入する場合→inletValueで決めた値に設定\n計算領域外に流出する場合→zeroGradient",
        "inletValue uniform 0; // ベクトルの場合は(1.2 3.4 5.6)のように書く．",
        f"{src}finiteVolume/fields/fvPatchFields/derived/inletOutlet",
    ),
    (
        "kqRWallFunction",
        "高レイノルズ数型乱流モデルにおけるk, q, Rの壁面境界条件\nzeroGrdientのラッパー",
        "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/kqRWallFunctions/kqRWallFunction",
    ),
    (
        "movingWallVelocity",
        "Uに使用\n移動壁面の場合のnoSlip条件，壁面が移動しなければnoSlipと同じ",
        "value uniform (0 0 0); // 初期速度",
        f"{src}finiteVolume/fields/fvPatchFields/derived/movingWallVelocity",
    ),
    (
        ...
        + "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/nutWallFunctions/nutkRoughWallFunction",
    ),
    (
        "nutkWallFunction",
        "nutの壁面境界条件，標準的\n"
        ...
        "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/nutWallFunctions/nutkWallFunction",
    ),
    (
        "nutUWallFunction",
        "nutの壁面境界条件\n"
        ...
        + "になるように乱流粘性係数を設定する．\nhttps://www.slideshare.net/fumiyanozaki96/openfoam-36426892",
        "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/nutWallFunctions/nutUWallFunction",
    ),
    (
        "noSlip",
        "U = (0 0 0)に規定",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/derived/noSlip",
    ),
    (
        "omegaWallFunction",
        "omegaの壁面境界条件",
        "value $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/derivedFvPatchFields/wallFunctions/omegaWallFunctions/omegaWallFunction",
    ),
    (
        "outletInlet",
        "計算領域外に流出する場合→outletValueで決めた値に設定\n計算領域内に流入する場合→zeroGradient",
        "outletValue uniform 0; // ベクトルの場合は(1.2 3.4 5.6)のように書く．",
        f"{src}finiteVolume/fields/fvPatchFields/derived/outletInlet",
    ),
    (
        ...
        "alpha alpha.water;\nUmean 1.2;",
        f"{src}finiteVolume/fields/fvPatchFields/derived/outletPhaseMeanVelocity",
    ),
    (
        ...
        f"{src}finiteVolume/fields/fvPatchFields/derived/pressureInletOutletVelocity",
    ),
    (
        "prghPressure",
        ...
        f"{src}finiteVolume/fields/fvPatchFields/derived/prghPressure",
    ),
    (
        ...
        f"{src}finiteVolume/fields/fvPatchFields/derived/rotatingWallVelocity",
    ),
    (
        "slip",
        "非粘性流れの壁面境界条件",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/derived/slip",
    ),
    (
        "surfaceNormalFixedValue",
        "境界に垂直な方向の速度を外向きを正として設定し，平行方向の速度は0にする．",
        "refValue uniform -10; // 垂直方向速度",
        f"{src}finiteVolume/fields/fvPatchFields/derived/surfaceNormalFixedValue",
    ),
    (
        "symmetry",
        "対称境界，境界が曲がっていても使える",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/constraint/symmetry",
    ),
    (
        "symmetryPlane",
        "対称境界，完全な平面にしか使えない",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/constraint/symmetryPlane",
    ),
    (
        "totalPressure",
        "pまたはp_rghに使用\np0で決めた値が全圧（p_rghの場合は全圧 + rho*g*z）になるように設定",
        "p0 uniform 0;",
        f"{src}finiteVolume/fields/fvPatchFields/derived/totalPressure",
    ),
    (
        "turbulentIntensityKineticEnergyInlet",
        "計算領域内に流入する場合→k = 1.5*(intensity*局所速度)^2に規定\n計算領域外に流出する場合→zeroGradient",
        "intensity 0.05;\nvalue $internalField; // 実際には使わないけど必要",
        f"{src}finiteVolume/fields/fvPatchFields/derived/turbulentIntensityKineticEnergyInlet",
    ),
    (
        "turbulentMixingLengthDissipationRateInlet",
        "計算領域内に流入する場合→epsilon = C_mu^0.75*k^1.5/混合距離, C_mu = 0.09に規定\n計算領域外に流出する場合→zeroGradient",
        "mixingLength 0.001; // 混合距離，ふつうは0.07*管内径ぐらいの大きさ\nvalue $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/RAS/derivedFvPatchFields/turbulentMixingLengthDissipationRateInlet",
    ),
    (
        "turbulentMixingLengthFrequencyInlet",
        "計算領域内に流入する場合→omega = k^0.5/(C_mu^0.25*混合距離), C_mu = 0.09に規定\n計算領域外に流出する場合→zeroGradient",
        "mixingLength 0.001; // 混合距離，ふつうは0.07*管内径ぐらいの大きさ\nvalue $internalField; // 実際には使わないけど必要",
        f"{src}TurbulenceModels/turbulenceModels/RAS/derivedFvPatchFields/turbulentMixingLengthFrequencyInlet",
    ),
    (
        ...
        "lowerBound 0.0;\nupperBound 1.0;",
        f"{src}finiteVolume/fields/fvPatchFields/derived/variableHeightFlowRate",
    ),
    (
        "zeroGradient",
        "こう配が0，境界での値 = セル中心での値にする．",
        "",
        f"{src}finiteVolume/fields/fvPatchFields/basic/zeroGradient",
    ),
)
...
def select_boundary_condition():
    ...
    while True:
        ...
        if n == 0:
            ...
            print(f"\033[4;5m{s[:-1]}")
            print(
                f"--> {selection[0]}の雛形をクリップボードにコピーしたければ，Enterを押して下さい．\033[m"
            )
        else:
            print(s[:-1])
        print(
            "\n上に挙げる境界条件の中からtypeの最初の数文字を入力して，候補を絞って下さい．"
            f"(Enterのみ: 終了, Tab: {p}まで書き込む) > {type_name}"
        )
        c = getch()
        oc = ord(c)
        if oc == 13:  # enter
    ...
    if len(sys.argv) > 1:
        ...
        while True:
            ...
            if selection is None:
                ...
                print(
                    "コピーしました．変数のファイルにペーストして，適切に書き換えて下さい．"
                )
