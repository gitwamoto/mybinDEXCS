...
def handler(signum, frame):
    if two_dimensional:
        ...
        if os.path.isfile(snappyHexMeshDict_3D_path):
            os.rename(
                snappyHexMeshDict_3D_path, snappyHexMeshDict_path
            )  # can overwrite
...
if __name__ == "__main__":
    ...
    # }
    CUSTOM_OPTIONS = snappyHexMeshDict.find_element(
        [{"type": "block", "key": "CUSTOM_OPTIONS"}]
    )["element"]
    makeBlockMeshDict(
        max_cell_size=float(
            CUSTOM_OPTIONS.find_element(
                [
        ...
        bounding_box=[
            float(i["element"]["value"])
            for i in CUSTOM_OPTIONS.find_all_elements(
                [
                    {"type": "dictionary", "key": "boundingBox"},
                    {"type": "list"},
                    {"type": "list"},
    ...
    for b in snappyHexMeshDict.find_all_elements(
        ...
        if b["key"].endswith(".stl"):  # STLファイルは1つにまとめられていると仮定
            ...
            stl_file_name = b["key"]
            n = b.find_element(
                [{"type": "dictionary", "key": "name"}, {"except type": "ignorable"}]
            )["element"]
            if n is not None:
                geometry_stl_name = n[
                    "value"
                ]  # 最終的なパッチ名は nameの値 + _ + STL内のsolid名 になります。
            break
    ...
    if two_dimensional:
        ...
        with open(os.path.join("constant", "triSurface", stl_2D_file_name), "w") as f:
            for line in open(
                os.path.join("constant", "triSurface", stl_file_name), "r"
            ):
    ...
    if two_dimensional:
        os.rename(
            snappyHexMeshDict_path, snappyHexMeshDict_path + "_2D"
        )  # can overwrite
    ...
    if two_dimensional:
        ...
        misc.execCommand(
            [
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "2次元メッシュに.py"
                ),
                "-f",
                front_name,
                "-b",
    ...
    if (
        snappyHexMeshDict.find_element(
            [
                {"type": "block", "key": "castellatedMeshControls"},
                {"type": "dictionary", "key": "locationsInMesh"},
            ]
        )["element"]
        is not None
    ):
