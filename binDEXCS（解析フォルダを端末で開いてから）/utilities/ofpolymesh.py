    ...
    def make_indices(self):
        ...
        for j in range(self.jmax):
            for i in range(self.imax):
                if (
                    i > 0
                    and self.x[i, j] == self.x[i - 1, j]
                    and self.y[i, j] == self.y[i - 1, j]
                ):
                    self.pidx[i, j] = self.pidx[i - 1, j]
                elif (
                    j > 0
                    and self.x[i, j] == self.x[i, j - 1]
                    and self.y[i, j] == self.y[i, j - 1]
                ):
        ...
        for i in range(1, self.imax - 1):
            for j in range(self.jmax - 1):
                if self.pidx[i, j + 1] != self.pidx[i, j]:
                    ...
                    self.fiidx[i, j, 2] = (
                        self.pidx[i, j + self.ca] + self.cb * self.nPoints
                    )
                    self.fiidx[i, j, 3] = self.pidx[i, j + 1] + self.nPoints
                    self.fiidx[i, j, 4] = (
                        self.pidx[i, j + self.cb] + self.ca * self.nPoints
                    )
        ...
        for j in range(1, self.jmax - 1):
            for i in range(self.imax - 1):
                if self.pidx[i + 1, j] != self.pidx[i, j]:
                    ...
                    self.fjidx[i, j, 2] = (
                        self.pidx[i + self.cb, j] + self.ca * self.nPoints
                    )
                    self.fjidx[i, j, 3] = self.pidx[i + 1, j] + self.nPoints
                    self.fjidx[i, j, 4] = (
                        self.pidx[i + self.ca, j] + self.cb * self.nPoints
                    )
                    self.nInternalFaces += 1
                else:
                    self.fjidx[i, j, 0] = 0
            ...
            if b.i0 == b.i1:
                ...
                for j in range(b.j0, b.j1 - 1):
                    if self.pidx[b.i0, j + 1] != self.pidx[b.i0, j]:
                        ...
                        self.fiidx[b.i0, j, 2] = (
                            self.pidx[b.i0, j + ca] + cb * self.nPoints
                        )
                        self.fiidx[b.i0, j, 3] = self.pidx[b.i0, j + 1] + self.nPoints
                        self.fiidx[b.i0, j, 4] = (
                            self.pidx[b.i0, j + cb] + ca * self.nPoints
                        )
                        b.nFaces += 1
                        self.nFaces += 1
                    else:
                ...
                for i in range(b.i0, b.i1 - 1):
                    if self.pidx[i + 1, b.j0] != self.pidx[i, b.j0]:
                        ...
                        self.fjidx[i, b.j0, 2] = (
                            self.pidx[i + cb, b.j0] + ca * self.nPoints
                        )
                        self.fjidx[i, b.j0, 3] = self.pidx[i + 1, b.j0] + self.nPoints
                        self.fjidx[i, b.j0, 4] = (
                            self.pidx[i + ca, b.j0] + cb * self.nPoints
                        )
                        b.nFaces += 1
                        self.nFaces += 1
                    else:
        ...
        for j in range(self.jmax - 1):
            for i in range(self.imax - 1):
                ...
                if (
                    self.pidx[i + self.ca, j + self.cb] != self.pidx[i + 1, j + 1]
                    and self.pidx[i + self.ca, j + self.cb] != self.pidx[i, j]
                ):
                    n += 1
        ...
        if self.ascii:
            with open(os.path.join(self.parent_dir_path, "points"), "w") as fp:
                ...
                for j in range(self.jmax):
                    for i in range(self.imax):
                        if self.pidx[i, j] == n:
                            fp.write(
                                f"({self.x[i, j]:#.{sys.float_info.dig}e} "
            ...
            with open(os.path.join(self.parent_dir_path, "faces"), "w") as ff:
                ...
                with open(os.path.join(self.parent_dir_path, "owner"), "w") as fo:
                    ...
                    with open(
                        os.path.join(self.parent_dir_path, "neighbour"), "w"
                    ) as fn:
                        fn.write(
                            "FoamFile\n"
                            "{\n"
                        ...
                        for i in range(1, self.imax - 1):
                            for j in range(self.jmax - 1):
                                if self.fiidx[i, j, 0] > 0:
                                    ff.write(
                                        f"{self.fiidx[i, j, 0]}({self.fiidx[i, j, 1]}"
                                    )
                        ...
                        for j in range(1, self.jmax - 1):
                            for i in range(self.imax - 1):
                                if self.fjidx[i, j, 0] > 0:
                                    ff.write(
                                        f"{self.fjidx[i, j, 0]}({self.fjidx[i, j, 1]}"
                                    )
                        ...
                        if b.i0 == b.i1:
                            for j in range(b.j0, b.j1 - 1):
                                if self.fiidx[b.i0, j, 0]:
                                    ff.write(
                                        f"{self.fiidx[b.i0, j, 0]}({self.fiidx[b.i0, j, 1]}"
                                    )
                                    for n in range(2, self.fiidx[b.i0, j, 0] + 1):
                                        ff.write(f" {self.fiidx[b.i0, j, n]}")
                                    ff.write(")\n")
                                    fo.write(
                                        f"{self.cindex(b.i0 - 1 if b.i0 > 0 else 0, j)}\n"
                                    )
                        else:  # b.j0 == b.j1
                            for i in range(b.i0, b.i1 - 1):
                                if self.fjidx[i, b.j0, 0] > 0:
                                    ff.write(
                                        f"{self.fjidx[i, b.j0, 0]}({self.fjidx[i, b.j0, 1]}"
                                    )
                                    for n in range(2, self.fjidx[i, b.j0, 0] + 1):
                                        ff.write(f" {self.fjidx[i, b.j0, n]}")
                                    ff.write(")\n")
                                    fo.write(
                                        f"{self.cindex(i, b.j0 - 1 if b.j0 > 0 else 0)}\n"
                                    )
                    # front
                    for j in range(self.jmax - 1):
                        for i in range(self.imax - 1):
                            if self.fkidx[i, j, 0] > 0:
                                ff.write(
                                    f"{self.fkidx[i, j, 0]}({self.fkidx[i, j, 1] + self.nPoints / 2}"
                                )
                                for n in range(self.fkidx[i, j, 0], 1, -1):
                                    ff.write(
                                        f" {self.fkidx[i, j, n] + self.nPoints / 2}"
                                    )
                                ff.write(")\n")
            ...
            with open(os.path.join(self.parent_dir_path, "points"), "wb") as fp:
                ...
                for j in range(self.jmax):
                    for i in range(self.imax):
                        if self.pidx[i, j] == n:
                            fp.write(
                                pack("<ddd", self.x[i, j], self.y[i, j], self.depth)
                            )
                            n += 1
                fp.write(")\n")
            ...
            with open(os.path.join(self.parent_dir_path, "faces"), "wb") as ff:
                ...
                with open(os.path.join(self.parent_dir_path, "owner"), "wb") as fo:
                    ...
                    with open(
                        os.path.join(self.parent_dir_path, "neighbour"), "wb"
                    ) as fn:
                        fn.write(
                            "FoamFile\n"
                            "{\n"
                    ...
                    for b in self.boundary:
                        if b.i0 == b.i1:
                            for j in range(b.j0, b.j1 - 1):
                                if self.fiidx[b.i0, j, 0] > 0:
                                    self.fiidx[
                                        b.i0, j, 1 : 1 + self.fiidx[b.i0, j, 0]
                                    ].astype("<I").tofile(ff)
                                    fo.write(
                                        pack(
                                            "<I",
                                            self.cindex(b.i0 - 1 if b.i0 > 0 else 0, j),
                                        )
                                    )
                        else:  # b.j0 == b.j1
                            for i in range(b.i0, b.i1 - 1):
                                if self.fjidx[i, b.j0, 0] > 0:
                                    self.fjidx[
                                        i, b.j0, 1 : 1 + self.fjidx[i, b.j0, 0]
                                    ].astype("<I").tofile(ff)
                                    fo.write(
                                        pack(
                                            "<I",
                                            self.cindex(i, b.j0 - 1 if b.j0 > 0 else 0),
                                        )
                                    )
                    # front
                    for j in range(self.jmax - 1):
                        for i in range(self.imax - 1):
                            if self.fkidx[i, j, 0] > 0:
                                ff.write(
                                    pack("<I", self.fkidx[i, j, 1] + self.nPoints / 2)
                                )
                                self.fkidx[i, j, self.fkidx[i, j, 0] : 1 : -1].astype(
                                    "<I"
                                ).tofile(ff)
                    ...
                    for j in range(self.jmax - 1):
                        for i in range(self.imax - 1):
                            if self.fkidx[i, j, 0] > 0:
                                self.fkidx[i, j, 1 : 1 + self.fkidx[i, j, 0]].astype(
                                    "<I"
                                ).tofile(ff)
                                fo.write(pack("<I", self.cindex(i, j)))
                    fo.write(")\n")
...
class boundary3D(object):
    def __init__(self, name, type, i0, i1, j0, j1, k0, k1):
        if i0 != i1 and j0 != j1 and k0 != k1:
            print(
                "Error: Relationship i0 = i1 or j0 = j1 or k0 = k1 should be satisfied."
            )
            sys.exit(1)
        self.i0 = i0
        self.i1 = i1
...
class structured3D(object):
    ...
    def set_boundary(
        self, name, type, i0=None, i1=None, j0=None, j1=None, k0=None, k1=None
    ):
        if i1 is None:
            if i0 is None:
                i0 = 0
        ...
        for k in range(self.kmax):
            for j in range(self.jmax):
                for i in range(self.imax):
                    if (
                        i > 0
                        and self.x[i, j, k] == self.x[i - 1, j, k]
                        and self.y[i, j, k] == self.y[i - 1, j, k]
                        and self.z[i, j, k] == self.z[i - 1, j, k]
                    ):
                        self.pidx[i, j, k] = self.pidx[i - 1, j, k]
                    elif (
                        j > 0
                        and self.x[i, j, k] == self.x[i, j - 1, k]
                        and self.y[i, j, k] == self.y[i, j - 1, k]
                        and self.z[i, j, k] == self.z[i, j - 1, k]
                    ):
                        self.pidx[i, j, k] = self.pidx[i, j - 1, k]
                    elif (
                        k > 0
                        and self.x[i, j, k] == self.x[i, j, k - 1]
                        and self.y[i, j, k] == self.y[i, j, k - 1]
                        and self.z[i, j, k] == self.z[i, j, k - 1]
                    ):
        ...
        for i in range(1, self.imax - 1):
            for k in range(self.kmax - 1):
                for j in range(self.jmax - 1):
                    ...
                    if (
                        self.pidx[i, j + 1, k + 1]
                        != self.pidx[i, j + self.ca, k + self.cb]
                    ):
                        n += 1
                        self.fiidx[i, j, k, n] = self.pidx[i, j + 1, k + 1]
                    if (
                        self.pidx[i, j + self.cb, k + self.ca]
                        != self.pidx[i, j + 1, k + 1]
                        and self.pidx[i, j + self.cb, k + self.ca] != self.pidx[i, j, k]
                    ):
                        n += 1
        ...
        for j in range(1, self.jmax - 1):
            for i in range(self.imax - 1):
                for k in range(self.kmax - 1):
                    ...
                    if (
                        self.pidx[i + 1, j, k + 1]
                        != self.pidx[i + self.cb, j, k + self.ca]
                    ):
                        n += 1
                        self.fjidx[i, j, k, n] = self.pidx[i + 1, j, k + 1]
                    if (
                        self.pidx[i + self.ca, j, k + self.cb]
                        != self.pidx[i + 1, j, k + 1]
                        and self.pidx[i + self.ca, j, k + self.cb] != self.pidx[i, j, k]
                    ):
                        n += 1
        ...
        for k in range(1, self.kmax - 1):
            for j in range(self.jmax - 1):
                for i in range(self.imax - 1):
                    ...
                    if (
                        self.pidx[i + 1, j + 1, k]
                        != self.pidx[i + self.ca, j + self.cb, k]
                    ):
                        n += 1
                        self.fkidx[i, j, k, n] = self.pidx[i + 1, j + 1, k]
                    if (
                        self.pidx[i + self.cb, j + self.ca, k]
                        != self.pidx[i + 1, j + 1, k]
                        and self.pidx[i + self.cb, j + self.ca, k] != self.pidx[i, j, k]
                    ):
                        n += 1
            ...
            if b.i0 == b.i1:
                ...
                for k in range(b.k0, b.k1 - 1):
                    for j in range(b.j0, b.j1 - 1):
                        ...
                        if (
                            self.pidx[b.i0, j + 1, k + 1]
                            != self.pidx[b.i0, j + ca, k + cb]
                        ):
                            n += 1
                            self.fiidx[b.i0, j, k, n] = self.pidx[b.i0, j + 1, k + 1]
                        if (
                            self.pidx[b.i0, j + cb, k + ca]
                            != self.pidx[b.i0, j + 1, k + 1]
                            and self.pidx[b.i0, j + cb, k + ca] != self.pidx[b.i0, j, k]
                        ):
                            n += 1
                ...
                for i in range(b.i0, b.i1):
                    for k in range(b.k0, b.k1 - 1):
                        ...
                        if (
                            self.pidx[i + 1, b.j0, k + 1]
                            != self.pidx[i + cb, b.j0, k + ca]
                        ):
                            n += 1
                            self.fjidx[i, b.j0, k, n] = self.pidx[i + 1, b.j0, k + 1]
                        if (
                            self.pidx[i + ca, b.j0, k + cb]
                            != self.pidx[i + 1, b.j0, k + 1]
                            and self.pidx[i + ca, b.j0, k + cb] != self.pidx[i, b.j0, k]
                        ):
                            n += 1
                ...
                for j in range(b.j0, b.j1 - 1):
                    for i in range(b.i0, b.i1 - 1):
                        ...
                        if (
                            self.pidx[i + 1, j + 1, b.k0]
                            != self.pidx[i + ca, j + cb, b.k0]
                        ):
                            n += 1
                            self.fkidx[i, j, b.k0, n] = self.pidx[i + 1, j + 1, b.k0]
                        if (
                            self.pidx[i + cb, j + ca, b.k0]
                            != self.pidx[i + 1, j + 1, b.k0]
                            and self.pidx[i + cb, j + ca, b.k0] != self.pidx[i, j, b.k0]
                        ):
                            n += 1
                    ...
                    for j in range(self.jmax):
                        for i in range(self.imax):
                            if self.pidx[i, j, k] == n:
                                fp.write(
                                    f"({self.x[i, j, k]:#.{sys.float_info.dig}e} "
                            ...
                            for k in range(self.self.kmax - 1):
                                for j in range(self.self.jmax - 1):
                                    if self.fiidx[i, j, k, 0] > 0:
                                        ff.write(
                                            f"{self.fiidx[i, j, k, 0]}({self.fiidx[i, j, k, 1]}"
                                        )
                            ...
                            for i in range(self.imax - 1):
                                for k in range(self.kmax - 1):
                                    if self.fjidx[i, j, k, 0] > 0:
                                        ff.write(
                                            f"{self.fjidx[i, j, k, 0]}({self.fjidx[i, j, k, 1]}"
                                        )
                            ...
                            for j in range(self.jmax - 1):
                                for i in range(self.imax - 1):
                                    if self.fkidx[i, j, k, 0] > 0:
                                        ff.write(
                                            f"{self.fkidx[i, j, k, 0]}({self.fkidx[i, j, k, 1]}"
                                        )
                            ...
                            for k in range(b.k0, b.k1 - 1):
                                for j in range(b.j0, b.j1 - 1):
                                    if self.fiidx[b.i0, j, k, 0] > 0:
                                        ff.write(
                                            f"{self.fiidx[b.i0, j, k, 0]}({self.fiidx[b.i0, j, k, 1]}"
                                        )
                                        for n in range(
                                            2, self.fiidx[b.i0, j, k, 0] + 1
                                        ):
                                            ff.write(f" {self.fiidx[b.i0, j, k, n]}")
                                        ff.write(")\n")
                                        fo.write(
                                            f"{self.cindex(b.i0 - 1 if b.i0 > 0 else 0, j, k)}\n"
                                        )
                        elif b.j0 == b.j1:
                            for i in range(b.i0, b.i1 - 1):
                                for k in range(b.k0, b.k1 - 1):
                                    if self.fjidx[i, b.j0, k, 0] > 0:
                                        ff.write(
                                            f"{self.fjidx[i, b.j0, k, 0]}({self.fjidx[i, b.j0, k, 1]}"
                                        )
                                        for n in range(
                                            2, self.fjidx[i, b.j0, k, 0] + 1
                                        ):
                                            ff.write(f" {self.fjidx[i, b.j0, k, n]}")
                                        ff.write(")\n")
                                        fo.write(
                                            f"{self.cindex(i, b.j0 - 1 if b.j0 > 0 else 0, k)}\n"
                                        )
                        else:  # b.k0 == b.k1
                            for j in range(b.j0, b.j1 - 1):
                                for i in range(b.i0, b.i1 - 1):
                                    if self.fkidx[i, j, b.k0, 0] > 0:
                                        ff.write(
                                            f"{self.fkidx[i, j, b.k0, 0]}({self.fkidx[i, j, b.k0, 1]}"
                                        )
                                        for n in range(
                                            2, self.fkidx[i, j, b.k0, 0] + 1
                                        ):
                                            ff.write(f" {self.fkidx[i, j, b.k0, n]}")
                                        ff.write(")\n")
                                        fo.write(
                                            f"{self.cindex(i, j, b.k0 - 1 if b.k0 > 0 else 0)}\n"
                                        )
                    fo.write(")\n")
                ff.write(")\n")
        else:  # not self.ascii
                                ...
                                for j in range(b.j0, b.j1 - 1):
                                    if self.fiidx[b.i0, j, k, 0] > 0:
                                        self.fiidx[
                                            ...
                                            j,
                                            k,
                                            1 : 1 + self.fiidx[b.i0, j, k, 0],
                                        ].astype("<I").tofile(ff)
                                        fo.write(
                                            pack(
                                                "<I",
                                                self.cindex(
                                                    b.i0 - 1 if b.i0 > 0 else 0, j, k
                                                ),
                                            )
                                        )
                        elif b.j0 == b.j1:
                                ...
                                for k in range(b.k0, b.k1 - 1):
                                    if self.fjidx[i, b.j0, k, 0] > 0:
                                        self.fjidx[
                                            ...
                                            b.j0,
                                            k,
                                            1 : 1 + self.fjidx[i, b.j0, k, 0],
                                        ].astype("<I").tofile(ff)
                                        fo.write(
                                            pack(
                                                "<I",
                                                self.cindex(
                                                    i, b.j0 - 1 if b.j0 > 0 else 0, k
                                                ),
                                            )
                                        )
                        else:  # b.k0 == b.k1
                                ...
                                for i in range(b.i0, b.i1 - 1):
                                    if self.fkidx[i, j, b.k0, 0] > 0:
                                        self.fkidx[
                                            ...
                                            j,
                                            b.k0,
                                            1 : 1 + self.fkidx[i, j, b.k0, 0],
                                        ].astype("<I").tofile(ff)
                                        fo.write(
                                            pack(
                                                "<I",
                                                self.cindex(
                                                    i, j, b.k0 - 1 if b.k0 > 0 else 0
                                                ),
                                            )
                                        )
                    fo.write(")\n")
