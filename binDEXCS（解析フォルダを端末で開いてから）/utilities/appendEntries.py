...
def intoFvSolution():
    ...
    def intoFvSolutionIn(path):
        ...
        if solvers is not None:
            ...
            if (
                i["element"] is not None
                and "数値的に安定だと思われる設定" in i["element"]["value"]
            ):
                s = re.search(
                    r"数値的に安定だと思われる設定\s*[(（]\s*([0-9/]+)\s*現在\s*[)）]",
                    i["element"]["value"],
                )
            ...
            if (
                i["element"] is None
                or "数値的に安定だと思われる設定" not in i["element"]["value"]
            ):
                solvers["value"][solvers_start:solvers_start] = dictParse.DictParser(
                    string="\n"
                    "/*\n"
        ...
        tail_index = (
            fvSolution.find_element(
                [{"except type": "whitespace|linebreak|separator"}],
                reverse=True,
                index_not_found=len(fvSolution["value"]) - 1,
            )["index"]
            + 1
        )

        # potentialFlow
        potentialFlow = fvSolution.find_element(
            [{"type": "block", "key": "potentialFlow"}]
        )["element"]
        if potentialFlow is None:
            linebreak_and_potentialFlow = dictParse.DictParser(
            ...
            if nNonOrthogonalCorrectors is None:
                block_end = potentialFlow.find_element(
                    [{"type": "block_end"}], reverse=True
                )
                block_end["parent"][block_end["index"] : block_end["index"]] = (
                    dictParse.DictParser(string="nNonOrthogonalCorrectors\t10;\n")[
                        "value"
                    ]
                )
        ...
        for k in ("SIMPLE", "PIMPLE", "PISO"):
            ...
            if block is None:
                linebreak_and_block = dictParse.DictParser(string=f"\n\n{k}\n{{\n}}")[
                    "value"
                ]
            ...
            block_start = (
                block.find_element(
                    [{"type": "linebreak"}], start=i, index_not_found=i - 1
                )["index"]
                + 1
            )

            if k == "SIMPLE":
                residualControl = block.find_element(
                    [{"type": "block", "key": "residualControl"}]
                )
                if residualControl["element"] is None:
                    block["value"][block_start:block_start] = dictParse.DictParser(
            ...
            if k != "PISO":
                consistent = block.find_element(
                    [{"type": "dictionary", "key": "consistent"}]
                )
            ...
            if nNonOrthogonalCorrectors["element"] is None:
                ...
                nNonOrthogonalCorrectors["parent"][block_start:block_start] = [
                    nNonOrthogonalCorrectors["parent"].pop(
                        nNonOrthogonalCorrectors["index"]
                    ),
                    dictParse.DictParser(string="\n")["value"][0],
                ]

            if k != "SIMPLE":
                nCorrectors = block.find_element(
                    [{"type": "dictionary", "key": "nCorrectors"}]
                )
            ...
            momentumPredictor = block.find_element(
                [{"type": "dictionary", "key": "momentumPredictor"}]
            )
            if momentumPredictor["element"] is None:
                v = "yes"
            else:
                v = momentumPredictor["element"].find_element(
                    [{"except type": "ignorable"}]
                )["element"]["value"]
                del block["value"][momentumPredictor["index"]]
            block["value"][block_start:block_start] = dictParse.DictParser(
                string=f"momentumPredictor\t{v};"
...
def intoFvSchemes():
    ...
    def intoFvSchemesIn(path):
        ...
        for b, k, v in (
            ...
            if block is None:
                linebreak_and_block = dictParse.DictParser(
                    string=f"\n\n{b}\n{{\n\t{k}\t{v};\n}}"
                )["value"]
                tail_index = (
                    fvSchemes.find_element(
                        [{"except type": "whitespace|linebreak|separator"}],
                        reverse=True,
                        index_not_found=len(fvSchemes["value"]) - 1,
                    )["index"]
                    + 1
                )
        ...
        if (
            ...
            print(
                f"\n\033[3;4;5m{application}に対して{fvSchemes_path}ファイルの"
                f"ddtSchemesは{ddtSchemes}にはならないと思うので，steadyStateに書き換えました．\033[m\n"
            )
        elif (
            ...
            print(
                f"\n\033[3;4;5m{application}に対して{fvSchemes_path}ファイルの"
                "ddtSchemesがsteadyStateにはならないと思うので，Eulerに書き換えました．\033[m\n"
            )
...
def intoControlDict():
    ...
    startFrom = controlDict.find_element(
        [{"type": "dictionary", "key": "startFrom"}, {"except type": "ignorable"}]
    )
    if startFrom["element"]["value"] != "latestTime":
        startFrom["parent"][startFrom["index"] : startFrom["index"] + 1] = (
            dictParse.DictParser(string="latestTime")["value"]
        )
        print(
            f"\n\033[3;4;5m{controlDict_path}ファイルのstartFromをlatestTimeに書き換えました．\033[m\n"
        )
    ...
    i = controlDict.find_element(
        [{"type": "block_comment"}], start=deltaT_index - 1, reverse=True
    )["element"]
    if i is None or "simpleFoam" not in i["value"]:
        controlDict["value"][deltaT_index:deltaT_index] = dictParse.DictParser(
            string="/*\n"
    ...
    for block in functions.find_all_elements([{"type": "block"}]):
        ...
        if enabled["element"] is None:
            ...
            v = enabled["element"].find_element([{"except type": "ignorable"}])[
                "element"
            ]["value"]
        ...
        if insertion["element"] is not None:
            ...
            insertion = (
                block.find_element([{"type": "dictionary", "key": "type"}])["index"] + 1
            )
        insertion = (
            block.find_element(
                [{"type": "linebreak"}], start=insertion, index_not_found=insertion - 1
            )["index"]
            + 1
        )
        block["value"][insertion:insertion] = dictParse.DictParser(
            string=f"enabled\t{v}; // yesで実行\n"
        )["value"]
        block.set_blank_line(number_of_blank_lines=0)
    ...
    runTimeModifiable = controlDict.find_element(
        [{"type": "dictionary", "key": "runTimeModifiable"}]
    )["element"]
    if runTimeModifiable is None:
        linebreak_and_runTimeModifiable = dictParse.DictParser(
            string="\nrunTimeModifiable\tyes;\n"
        )["value"]
        controlDict["value"][functions_index:functions_index] = (
            linebreak_and_runTimeModifiable
        )
