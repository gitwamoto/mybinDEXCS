...
def normalize(file_name=None, string=None, overwrite_file=True):
    ...
    if sys.version_info.major <= 2:
        normalized_string = re.sub(
            ...
            re.sub(
                "(?<=[;{}()\\[\\]][ \\t])[ \\t]+(?=\\S)",
                "",  # セミコロン，かっこの後ろの余計なスペースを削除
                re.sub(
                    "(?<＝[\\u3040-\\u309F\\u30A0-\\u30FF\\u4E00-\\u9FFF])[,.]",
                    lambda m: unichr(
                        ord(m.group()) + 0xFEE0
                    ),  # 全角ひらがな，カタカナ，漢字以外に続く「．」と「，」を半角に
                    re.sub(
                        "[！-～]",
                        lambda m: unichr(
                            ord(m.group()) - 0xFEE0
                        ),  # 英数字・記号を半角に変換
                        string.decode("UTF-8").replace(
                            "　", " "
                        ),  # 全角スペースを半角に
                        flags=re.UNICODE,
                    ),
...
def re_findall_except_comments(pattern, string):
    ...
    while index < len(string):
        ...
        if substring_start != s.start():
            result.extend(
                pat_findall.findall(string, pos=substring_start, endpos=s.start())
            )
        index = substring_start = s.end()
    return result
...
def find_element(
    parent, path_list, start=None, end=None, reverse=False, index_not_found=None
):
    ...
    p = {
        k: v.replace(
            "ignorable", "whitespace|linebreak|line_comment|block_comment"
        ).split("|")
    ...
    c = next(
        (
            ...
            if all(
                (parent[i].get(k.replace("except ", "")) in p[k])
                != k.startswith("except ")
                for k, v in p.items()
            )
        ),
...
def find_all_elements(parent, path_list):
    ...
    c = [
        ...
        if all(
            (parent[i].get(k.replace("except ", "")) in p[k]) != k.startswith("except ")
            for k, v in path_list[0].items()
        )
    ]
    if len(c) == 0 or len(path_list) == 1:
        return [
            {"parent": parent, "index": i["index"], "element": i["element"]} for i in c
        ]
    else:
        elements = []
        for i in c:
...
def structure_string(parent, parent_header="", indent_level=0):
    if parent is None:
        return None
    elif isinstance(parent, DictParser) and parent["type"] != "root":
        parent = [parent]
    elif not isinstance(parent, list):
        parent = parent["value"]
    ...
    for n, i in enumerate(parent):
        ...
        if i["type"] in ("dictionary", "block"):
            s += (
                ...
                + f"'value':\n{structure_string(i['value'], h, indent_level + 1)}}}\n"
            )
        elif i["type"] == "list":
            s += (
                "{'type': 'list', "
            ...
            s += f"{i}\n"
    return s + (
        parent_header + "  " + "  " * (indent_level - 1) if indent_level > 0 else ""
    )
    ...
    for i in parent:
        if pretty_print and linebreak:  # 改行直後
            if (
                i["type"] == "whitespace"
                or commentless
                and i["type"] in ("line_comment", "block_comment")
            ):
                continue
        ...
        if i["type"] in ("block", "list", "dimension"):
            start = (
                find_element(i["value"], [{"type": i["type"] + "_start"}])["index"] + 1
            )
            end = find_element(
                i["value"], [{"type": i["type"] + "_end"}], reverse=True
            )["index"]
            j = find_element(
                i["value"],
                [{"except type": "whitespace"}],
                start=end - 1,
                end=start - 1,
                reverse=True,
            )
            if j["element"] is not None:
                    ...
                    j["index"]
                    if j["element"]["type"] == "linebreak"
                    else len(i["value"])
                )
                s += (
                    f"{i.get('key', '')}{i.get('length', '')}"
                    + file_string(
            ...
            if commentless and i["type"] == "block_comment":
                ...
                if isinstance(i["value"], list):
                    s += f"{i.get('key', '')}{i.get('length', '')}" + file_string(
                        i["value"], indent_level, pretty_print, commentless
                    )
                else:
                    s += i["value"]
...
class DictParser(UserDict):
    ...
    def __init__(self, file_name=None, string=None, element=None):
        ...
        if file_name is not None:
            ...
            self.string = string
        super().__init__(
            {"type": "root", "value": self.elements_list()[0]}
            if element is None
            else element
        )
    ...
    def elements_list(self, index=0, terminator=None, essentials_required=0):
        ...
        def exit_with_message(message, last_index):
            print(
                ...
                + (
                    ...
                    else f" (File: {os.path.basename(self.file_name)})"
                )
        ...
        if debug:
            print(
                f"elements_list from {index}, terminator = {terminator}, "
        ...
        while index < len(self.string):
            ...
            if terminator_reached and s.lastgroup not in (
                "whitespace",
                "line_comment",
                "block_comment",
            ):
            ...
            if s.lastgroup == "directive":
                ...
                l.append(
                    DictParser(
                        element={"type": s.lastgroup, "key": s.group(), "value": v}
                    )
                )
                if debug:
                    print(f"    -> {l[-1]}")
                ...
                for i in range(len(l)):
                    if l[i]["type"] in ("word", "string"):
                        v, index = self.elements_list(
                            index=s.end(), terminator="reached"
                        )
                        l[i:] = [
                            DictParser(
                                element={
                ...
                if not dictionary:  # meaningless semicolon
                    l.append(
                        DictParser(element={"type": s.lastgroup, "value": s.group()})
                    )
                    index = s.end()
                if debug:
                    print(f"    -> {l[-1]}")
                ...
                v, index = self.elements_list(
                    index=s.end(), terminator=type_string + "_end"
                )
                has_header = False
                ...
                l.append(
                    DictParser(
                        element={
                            ...
                            "value": [
                                DictParser(
                                    element={"type": s.lastgroup, "value": s.group()}
                                )
                            ]
                            + v,
                        }
                ...
                if s.lastgroup != terminator:
                    exit_with_message(
                        f'Inappropriate closing bracket, "{self.CLOSING_SYMBOL[terminator]}" is required.',
                        s.end(),
                    )
                ...
                if s.lastgroup == "line_comment" and re.match(
                    r"// (?:\* ?)+//$", s.group()
                ):
                    l.append(
                        DictParser(element={"type": "separator", "value": s.group()})
                    )
                else:
                    l.append(
                        DictParser(element={"type": s.lastgroup, "value": s.group()})
        ...
        if not terminator_reached and terminator is not None:
            exit_with_message(
                f'Missing "{self.CLOSING_SYMBOL[terminator]}".', len(self.string)
            )
        if debug:
            print(f"    return {l}")
        return l, index
    ...
    def find_element(
        self, path_list, start=None, end=None, reverse=False, index_not_found=None
    ):
        return find_element(
            self,
            path_list,
    ...
    def find_separators(self, header_index_not_found=None, footer_index_not_found=None):
        ...
        if len(separators) == 0:
            return [
                {"parent": None, "index": header_index_not_found, "element": None},
                {"parent": None, "index": footer_index_not_found, "element": None},
            ]  # header, footer
        if (
            self.find_element(
                [{"except type": "ignorable"}], start=separators[-1]["index"] + 1
            )["element"]
            is not None
        ):
            return [
    ...
    def set_blank_line(self, number_of_blank_lines=1):
        ...
        if self["type"] in ("block", "list", "dimension"):
            ...
            end = self.find_element([{"type": self["type"] + "_end"}], reverse=True)[
                "index"
            ]
            parent = self["value"]
            ...
            start = i + 1
            i = find_element(
                parent, [{"except type": "whitespace|linebreak"}], start=start, end=end
            )["index"]
            if i is not None:
                del parent[start:i]
                end += start - i
        ...
        while i < end:
            if (
                parent[i]["type"] == "linebreak"
                and i > start
                and parent[i - 1]["type"] not in ("line_comment", "block_comment")
            ):
                linebreak = parent[i]
