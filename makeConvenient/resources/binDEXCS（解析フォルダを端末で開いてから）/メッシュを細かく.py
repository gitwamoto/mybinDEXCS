...
if __name__ == "__main__":
        ...
        while i < len(sys.argv):
            if sys.argv[i] == "-xr":
                try:
                    ...
                    print(
                        "エラー: -xrで指定されたx_maxまたはx_minが数値ではありません．"
                    )
                    sys.exit(1)
            elif sys.argv[i] == "-yr":
                try:
                    ...
                    print(
                        "エラー: -yrで指定されたy_maxまたはy_minがが数値ではありません．"
                    )
                    sys.exit(1)
            elif sys.argv[i] == "-zr":
                try:
                    ...
                    print(
                        "エラー: -zrで指定されたz_maxまたはz_minがが数値ではありません．"
                    )
                    sys.exit(1)
            elif sys.argv[i] == "-xf":
                x_fine = True
    ...
    if interactive:
        x_fine = (
            True
            if input("x方向に細かくしますか ？ (y/n) > ").strip().lower() == "y"
            else False
        )
        y_fine = (
            True
            if input("y方向に細かくしますか ？ (y/n) > ").strip().lower() == "y"
            else False
        )
        z_fine = (
            True
            if input("z方向に細かくしますか ？ (y/n) > ").strip().lower() == "y"
            else False
        )
