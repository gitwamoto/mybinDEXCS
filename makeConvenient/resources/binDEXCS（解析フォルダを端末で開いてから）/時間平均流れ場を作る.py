    ...
    for field in misc.volFieldList(misc.latestTime()):
        string += (
            ...
            f"{
                '変動^2の平均'
                if field != 'U'
                else '変動速度相関の平均（レイノルズ応力に-1をかけたもの），uu, uv, uw, vv, vw, wwの順で出力'
            }，onで計算"
            "\t\t\t\t\n"
            "\t\t\t\tbase\ttime;\n"
            "\t\t\t}\n"
...
if __name__ == "__main__":
    ...
    fieldAverage_related_files_txt = os.path.join(
        "postProcessing", "_fieldAverage_related_files.txt"
    )
    if os.path.isdir("postProcessing") and os.path.isfile(
        fieldAverage_related_files_txt
    ):
    ...
    if interactive:
        fieldAverage_is_written = (
            ...
            if input(
                ...
                "functionsにfieldAverageに関する指示が書き込まれていますか？ (y/n) > "
            )
            .strip()
            .lower()
            == "y"
    ...
    properties_list = [
        f"{i['parent']['key']}Properties"
        for i in types
        if i["element"].find_element([{"type": "word"}])["element"] == "fieldAverage"
    ]
    if len(properties_list) == 0:
        print(
            f"エラー: {controlDict_path}ファイルでfieldAverageに関する指示がありません．"
        )
        sys.exit(1)
    ...
    for properties in properties_list:
        ...
        if interactive:
            delete_except_for_newest_folder = (
                True
                if input(
                    f"{longest}以外のフォルダにある平均データを消しますか？"
                    " (y/n, 多くの場合yのはず) > "
                )
                .strip()
                .lower()
