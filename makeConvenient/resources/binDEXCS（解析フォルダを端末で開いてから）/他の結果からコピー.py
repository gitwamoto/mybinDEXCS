...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if source_path is None:
            print(
                "エラー: -sオプションを使ってコピー元となる解析フォルダのパスを入力して下さい．"
            )
            sys.exit(1)
    ...
    if startFrom == "latestTime":
        ...
        start_from = controlDict.find_element(
            [{"type": "dictionary", "key": "startTime"}, {"except type": "ignorable"}]
        )["element"]
        if start_from is None:
        ...
        print(
            f"エラー: {controlDict_path}ファイルのstartFromの指定が正しくありません．"
        )
        sys.exit(1)

    print(
        f"現在の解析フォルダにある{controlDict_path}ファイルに書いてあるstartFrom = "
    ...
    if interactive:
        source_path = input(
            "コピー元となる解析フォルダのパスを入力して下さい． > "
        ).strip()

    if (
        misc.execCommand(
            ["mapFields", "-consistent", "-sourceTime", "latestTime", source_path]
        )[1]
        != 0
    ):
        sys.exit(1)
