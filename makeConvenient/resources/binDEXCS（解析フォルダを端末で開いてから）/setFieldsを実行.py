...
if __name__ == "__main__":
    ...
    if interactive:
        setFieldsDict_is_written = (
            (
                True
                if input(
                    f"{setFieldsDict_path}ファイルの内容を確認して下さい．"
                    "設定値に関する指示が書き込まれていますか？ (y/n) > "
                )
                .strip()
                .lower()
                == "y"
    ...
    if not setFieldsDict_is_written:
        ...
        misc.execCommand(
            [os.path.join(binDEXCS_path, "texteditwx.py"), setFieldsDict_path]
        )

    setFieldsDict = dictParse.DictParser(file_name=setFieldsDict_path)

    fieldValues = setFieldsDict.find_all_elements(
        [
            {"type": "dictionary", "key": "defaultFieldValues"},
            {"type": "list"},
        ...
        [
            ...
            {"type": "list"},
            {"type": "block"},
            {"type": "dictionary", "value": "fieldValues"},
            {"type": "list"},
            {"except type": "ignorable|list_start|list_end"},
        ]
    )
    fields_set = sorted(
        {fieldValues[i]["element"]["value"] for i in range(1, len(fieldValues), 3)}
    )  # list
    ...
    print(
        "setFieldsで書き換えられる前のファイルは，末尾に.origをつけた名前でバックアップされています．"
    )
