...
def decode_if_necessary(s):
    return (
        ...
        if sys.version_info.major <= 2 and type(s) is str
        else s
    )
...
def encode_if_necessary(s):
    return (
        s.encode("CP932" if sys.platform == "win32" else "UTF-8")
        if sys.version_info.major <= 2 and type(s) is unicode
        else s
    )
...
gettext.translation(
    ...
    localedir=os.path.join(
        os.path.dirname(os.path.realpath(decode_if_necessary(__file__))), "locale"
    ),
    languages=languages,
    fallback=True,
).install()
...
plt.rcParams["axes.formatter.limits"] = [
    -3,
    4,
]  # [m, n] -> Scientific notation is used for data < 10^m or 10^n <= data
...
pat_math = re.compile(
    r"(?<![a-zA-Z0-9_.\s])\s*((?!(?:abs|max|min)\s*\()[a-z0-9]+\s*\(|(?:pi|e)(?![a-zA-Z0-9_.(]))"
)
pat_eq_plot = re.compile(
    r"(?:\s+|)(.+?)\s*,\s*([^,=]+?)\s*=\s*\[\s*(.+?)\s*,\s*(.+?)\s*\]\s*(L?/)\s*(.+?)\s*"
...
def get_file_from_google_drive(file_id):
    r = requests.get(
        "https://drive.google.com/uc", params={"export": "download", "id": file_id}
    )
    if r.ok:
        if b"Google Drive - Virus scan warning" in r.content:
            ...
            if cookies:
                ...
                m = re.search(
                    b'id="downloadForm" action=".+?&amp;confirm=(.+?)"', r.content
                )
                if m:
                    code = m[1]
                else:
            ...
            r = requests.get(
                ...
                params={"export": "download", "confirm": code, "id": file_id},
                cookies=cookies,
            )
            if not r.ok:
                return None
        return (
            r.content,
            r.apparent_encoding,
        )  # type(r.content) = str (Python 2); bytes (Python 3)
    else:
        return None
...
def naca_4digits_airfoil(digits, points=51):
    ...
    yt = (
        ...
        * (
            0.2969 * np.sqrt(x)
            + (-0.126 + (-0.3516 + (0.2843 - 0.1015 * x) * x) * x) * x
        )
    )
    yc = np.where(
        x < p,
        m / p**2 * (2.0 * p - x) * x,
        m / (1.0 - p) ** 2 * (1.0 - 2.0 * p + (2.0 * p - x) * x),
    )
    theta = np.arctan(
        np.where(x < p, 2.0 * m / p**2 * (p - x), 2.0 * m / (1.0 - p) ** 2 * (p - x))
    )
    yts = yt * np.sin(theta)
    ytc = yt * np.cos(theta)
    yts[-1] = ytc[-1] = 0.0
...
def time_str_a_is_newer_than_b(a, b):
    reg = re.compile(
        r"([0-9]+)\s*[/\-]\s*([0-9]+)\s*[/\-]\s*([0-9]+)\s+([0-9]+)\s*:\s*([0-9]+)\s*(?::\s*([0-9]+)\s*)?([AaPp][Mm])*"
    )
    ra = reg.search(a)
    rb = reg.search(b)
...
def save_and_clip_png(png_file_name, show=True):
    ...
    if show:
        if sys.platform == "win32":
            ...
            subprocess.call('open "{}"'.format(png_file_name), shell=True)
        else:
            subprocess.call('xdg-open "{}"'.format(png_file_name), shell=True)


def make_gif_from_pngs(
    png_file_lists, gif_file_name, duration_ms=400, loop=0, remove=False, show=True
):
    gif_file_name = correct_file_name_in_unicode(gif_file_name)  # unicode
    if not gif_file_name.endswith(".gif"):
        gif_file_name += ".gif"
    images = [
        Image.open(correct_file_name_in_unicode(png_file))
    ...
    if show:
        if sys.platform == "win32":
            ...
            subprocess.call('open "{}"'.format(gif_file_name), shell=True)
        else:
            subprocess.call('xdg-open "{}"'.format(gif_file_name), shell=True)
...
def data_from_file(
    ...
    if file_name.endswith(".csv"):
        ...
        for i in columns:
            ...
            while True:
                ...
                if r is not None:
                    s[-1] += i[: r.start()] + "("
                    s.append(
                        [int(r[1]) - 1, alphabet_to_number(r[2]) - 1, int(r[3]) - 1]
                    )
                    s.append(")")
                    if r.end() == len(i):
                        break
        ...
        if file_name.endswith(".xls"):
            ...
            lines = [
                [[v.value for v in row] for row in ws.rows] for ws in wb.worksheets
            ]
            wb.close()
        n_lines = None
        for i in columns1:
            for j in i:
                if type(j) is list:
                    n_lines = (
                        len(lines[j[0]]) - j[2]
                        if n_lines is None
                        else min(n_lines, len(lines[j[0]]) - j[2])
                    )
        for n in range(n_lines):
            stop = has_None = False
            for i in range(len(columns1)):
                try:
                    ...
                    for j in range(len(columns1[i])):
                        if type(columns1[i][j]) is list:
                            v = lines[columns1[i][j][0]][columns1[i][j][2]][
                                columns1[i][j][1]
                            ]
                            if sys.version_info.major <= 2:
        ...
        for line in open(file_name, "rb"):
            ...
            if line == "" or skip is not None and line.startswith(skip):
                continue
...
def common_setting(
    ...
    if grids is not None:
        if grids[0]:
            plt.gca().grid(
                which="major", axis="x", color="black", linestyle="--", linewidth=1.0
            )
            plt.gca().grid(
                which="minor", axis="x", color="black", linestyle="--", linewidth=0.5
            )
        if grids[1]:
            plt.gca().grid(
                which="major", axis="y", color="black", linestyle="--", linewidth=1.0
            )
            plt.gca().grid(
                which="minor", axis="y", color="black", linestyle="--", linewidth=0.5
            )
...
def insert_text(
    ...
    plt.text(
        ...
        transform=plt.gca().transAxes if coordinate == "axes" else plt.gca().transData,
        zorder=zorder,
    )
...
def insert_naca_4digits_airfoil(
    digits,
    xle,
    yle,
    xte,
    ...
    xy = np.hstack(
        (
            (xle + xy[:, 0] * c - xy[:, 1] * s).reshape(-1, 1),
            (yle + xy[:, 0] * s + xy[:, 1] * c).reshape(-1, 1),
        )
    )
    plt.gca().add_patch(
        patches.Polygon(
...
def plot_scatter(
    ...
    if line_style is not None:
        plt.plot(
            ...
            linestyle=line_style,
            color=line_color,
            label=label_in_legend if marker is None else None,
            zorder=zorder,
        )
    if marker is not None:
        try:
            if isinstance(marker_color, (tuple, list)) and not isinstance(
                marker_color[0], (tuple, list)
            ):
                                ...
                                cmap=plt.get_cmap(
                                    ...
                                    if paint == "rainbow"
                                        ...
                                        if paint == "wb"
                                            ...
                                            "gray"
                                            if paint == "bw"
                                            else ("Reds" if paint == "wR" else "Blues")
                                        )
                                    )
                                ),
    ...
    if label_in_legend is not None:
        if legend_location is not None:
            plt.legend(
                bbox_to_anchor=legend_location, loc="upper left", borderaxespad=0
            )
        else:
            plt.legend()
...
def plot_vector(
            ...
            if log_scale[2]:
                ...
                cbar = plt.colorbar(
                    ...
                    ticks=ticker.LogLocator(
                        base=10.0 if ticks[2] is None else ticks[2]
                    ),
                    format=ticker.LogFormatterMathtext(labelOnlyBase=True),
                )
                t = np.array(
                    [
                        [i + j for j in np.log10(np.arange(1, 9))]
                        for i in range(
                            int(math.floor(log10_l)), int(math.ceil(log10_r))
                        )
                    ]
                )
                t.reshape(-1)
                ...
                cbar.ax.yaxis.set_minor_locator(
                    ticker.FixedLocator(
                        np.arange(t, r - l - 0.1 * t, t) / (r - l)
                        if sys.version_info.major <= 2
                        else np.arange(t, r - l - 0.1 * t, t)
                    )
                )
        except:
            print(sys.exc_info())
    ...
    if legend is not None and sys.version_info.major > 2:
        ...
        plt.quiverkey(
            ...
            Y=legend[1],
            U=legend[2],
            label=("" if legend[2] == 1.0 else "{:g}".format(legend[2]))
            + (legend[3] if legend[3] is not None else ""),
            coordinates="axes",
            labelpos="E",
        )
    ...
    try:
        if structured_grid is None:
            ...
            if structured_grid[0] == -1 and structured_grid[1] == -1:
                ...
                for i in range(1, data.size):
                    ...
                    if (
                        math.fabs(
                            math.atan2(dx0 * dy1 - dy0 * dx1, dx0 * dx1 + dy0 * dy1)
                        )
                        > 0.001
                    ):
        ...
        if log_scale[2]:
            ...
            if structured_grid is None:
                if show_triangle:
                    plt.triplot(
                        triang, color="dimgray", linewidth=0.5, zorder=zorder + 0.2
                    )
                plt.tricontour(
                    triang,
                    data[2],
            ...
            if paint is not None:
                if structured_grid is None:
                    ...
                    plt.tricontourf(
                        triang,
                        data[2],
                        np.linspace(l, r, 256) if smooth_paint else t,
                        cmap=cmap,
                        extend="both",
                        zorder=zorder,
...
def plot(dict_lists, show=True):
    ...
    dict_lists[0]["fig_size"] = dict_lists[0]["aspect"] = dict_lists[0][
        "graph_edges"
    ] = None
    ...
    for i in dict_lists[1:]:
            ...
            if "file_name" in i:
                ...
                if "err_columns" in i and i["err_columns"] is not None:
                    ...
                    for j, k in enumerate(i["err_columns"]):
                        if type(k) is tuple:
                            k = [
                                l for l in sorted(set(k), key=k.index) if l is not None
                            ]
                            if len(k) == 0:
        ...
        if i["type"] == "scatter":
            try:
                for j in plot_scatter.__code__.co_varnames[
                    : plot_scatter.__code__.co_argcount
                ]:
                    if j in i:
                        d[j] = i[j]
            except:
            ...
            try:
                for j in plot_vector.__code__.co_varnames[
                    : plot_vector.__code__.co_argcount
                ]:
                    if j in i:
                        d[j] = i[j]
            except:
            ...
            try:
                for j in plot_contour.__code__.co_varnames[
                    : plot_contour.__code__.co_argcount
                ]:
                    if j in i:
                        d[j] = i[j]
            except:
    ...
    if "png_file_name" in dict_lists[0] and dict_lists[0]["png_file_name"] is not None:
        ...
        if sys.platform == "win32":
            with wx.MessageDialog(
                None,
                _(
                    "この後に現れる（Figure 1というタイトルを持つ）グラフウインドウは，ウインドウが閉じられるまで処理をブロックします！\n"
                    + "グラフを描きなおしたい場合は，その前に必ずグラフウインドウを閉じて下さい．"
                ),
                _("処理をブロックします！"),
...
def evaluate_plot_settings(s, dir_name="."):
    ...
    for i in range(len(s)):
        ...
        if "png_file_name" in s[i] and s[i]["png_file_name"] is not None:
            s[i]["png_file_name"] = os.path.normpath(
                os.path.join(dir_name, s[i]["png_file_name"])
            )
        elif "file_name" in s[i] and s[i]["file_name"] is not None:
            s[i]["file_name"] = os.path.normpath(
                os.path.join(dir_name, s[i]["file_name"])
            )
        if i == 0:
            continue
        try:
            ...
            if len(i) < 6:  # サイズ指定がない頃のサポート
                l.append(
                    (
                        ...
                        "right"
                        if i[2].startswith("r")
                        else ("center" if i[2].startswith("c") else "left"),
                        i[3].decode("UTF-8") if sys.version_info.major <= 2 else i[3],
                        plt.rcParams["font.size"],
                        i[4] if len(i) > 4 else zorder_others,
                    )
                )
                ...
                l.append(
                    (
                        ...
                        "right"
                        if i[3].startswith("r")
                        else ("center" if i[3].startswith("c") else "left"),
                        i[4].decode("UTF-8") if sys.version_info.major <= 2 else i[4],
                        i[5],
                        i[6],
                    )
                )
    ...
    if "param_dict" in s[0] and s[0]["param_dict"] is not None:
        for i in list(s[0]["param_dict"]):
            s[0]["param_dict"][
                i.decode("UTF-8") if sys.version_info.major <= 2 else i
            ] = s[0]["param_dict"].pop(i)
...
def make_script(s, file_name):
    ...
    with codecs.open(file_name, "w", encoding="UTF-8") as f:
        ...
        f.write("import sys\n")
        f.write("if sys.platform == 'darwin':\n")
        f.write("    import unicodedata\n")
        f.write("import os\n")
        f.write("def decode_if_necessary(s):\n")
        f.write(
            "    return s.decode('CP932' if sys.platform == 'win32' else 'UTF-8') if sys.version_info.major <= 2 else s\n"
        )
        f.write(
            "cwd = os.path.dirname(os.path.realpath(decode_if_necessary(__file__)))\n"
        )
        f.write("if sys.platform == 'darwin':\n")
        f.write("    # 濁点なし文字と濁点に分離されている文字->濁点付きの文字\n")
        f.write("    cwd = unicodedata.normalize('NFC', cwd)\n")
        f.write("elif sys.platform == 'win32':\n")
        f.write("    cwd = cwd.replace(os.sep, os.altsep)\n")
        f.write("os.chdir(cwd) # 作業フォルダをこのファイルがあるフォルダにする\n\n")
        import_file = correct_file_name_in_unicode(
            os.path.realpath(decode_if_necessary(__file__))
        )
        f.write(
            "##### %sがある場所，必要に応じて書き換える #####\n"
            % os.path.basename(import_file)
        )
        f.write("sys.path.append(u'%s')\n" % os.path.dirname(import_file))
        ...
        f.write("    # プロット，画像を表示させたい場合はshow = Trueにする\n")
        f.write(
            "    %s.plot(%s.evaluate_plot_settings(s, u'.'), show = False)\n"
            % (module_name, module_name)
        )
        f.write("    # gifアニメーションを作る\n")
        f.write(
            "#    "
            + module_name
            + ".make_gif_from_pngs(\n"
            + "#        png_file_lists = [u'a1.png', u'a2.png', ...], # 元データであるpngファイルのリスト，要変更\n"
            + "#        gif_file_name = u'a.gif', # 作られるgifアニメーションファイル名，要変更\n"
            + "#        duration_ms = 400, # 1コマあたりの時間 [ms]，必要に応じて変更\n"
            + "#        loop = 0, # 繰り返し回数，必要に応じて変更\n"
            + "#        remove = False, # 元データであるpngファイルを消したいときはTrueにする\n"
            + "#        show = False # gifアニメーションを表示させたい場合はTrueにする\n"
            + "#    )\n"
        )


def make_template(file_name="template.txt"):
    file_name = correct_file_name_in_unicode(file_name)  # unicode
    ...
    with codecs.open(file_name, "w", encoding="UTF-8") as f:
        ...
        f.write("    'type': 'global',\n")
        f.write(
            "# 'png_file_name': 'figure.png'\n"
            + "#     → figure.png という名前のpng画像ファイルにグラフを保存する，このファイルからの相対パス\n"
        )
        f.write(
            "# 'png_file_name': None # → 画像ファイルを保存せず，matplotlibのビューワーにグラフを表示する\n"
        )
        f.write("    'png_file_name': '%s_example.png',\n" % s)
        f.write("# 'fig_size': None # → 図の大きさを指定しない\n")
        f.write("# 'fig_size': (4, 4) # → 図の大きさ縦4インチ，横4インチにする\n")
        f.write("    'fig_size': None,\n")
        f.write("# データファイル中で読み飛ばす行の先頭文字\n")
        f.write("    'skip': '#',\n")
        f.write("# 'delimiter': None # → 半角スペースかタブ区切りでデータを読み込む\n")
        f.write("# 'delimiter': ',' # → コンマ区切りでデータを読み込む\n")
        f.write("    'delimiter': None,\n")
        f.write("# 'terminator': None # → 最後の行まで読み込む\n")
        f.write(
            "# 'terminator': '!!STOP' # → !!STOPという文字が含まれる行が出てきたら読み込み終了\n"
        )
        f.write("    'terminator': '!!STOP',\n")
        f.write(
            "# 'ranges': (None, (-1, 2), (-3, 8))\n"
            + "#     → x軸の範囲を指定せず，y, z軸の範囲をそれぞれ-1 ≤ y ≤ 2, -3 ≤ z ≤ 8にする\n"
        )
        f.write("    'ranges': (None, None, None),\n")
        f.write(
            "# 'ticks': (None, 0.5, 1) # → x軸の刻みを指定せず，y, z軸の刻みをそれぞれ0.5, 1にする\n"
        )
        f.write("    'ticks': (None, None, None),\n")
        f.write("# 'log_scale': (False, True, True) # → y, z軸を対数軸にする\n")
        f.write("    'log_scale': (False, False, False),\n")
        f.write("# x, y, z軸のラベル，$\\mathsf{ と }$ の間に書くとTeX形式になる\n")
        f.write("    'labels': (r'x', r'y', r'z'),\n")
        f.write(
            "# x, y軸の目盛線，(True, False) → x軸の目盛線を長く描き，y軸は刻みだけ描く\n"
        )
        f.write("    'grids': (False, False),\n")
        f.write("# 'aspect': 'auto' # → グラフの縦横比を特に指定しない\n")
        f.write("# 'aspect': 'equal' # → x, y軸方向に1進む長さを等しくする\n")
        f.write("# 'aspect': 3.0 # → y軸方向に1進む長さをx軸のそれの3.0倍にする\n")
        f.write("    'aspect': 'equal',\n")
        f.write(
            "# 'title': 'graph for $\\mathsf{a_{xy}^2}$'\n"
            + "#     → グラフの上に書く題名，$\\mathsf{ と }$ の間に書くとTeX形式になる\n"
        )
        f.write("# 'title': None # → グラフの上に題名を書かない\n")
        f.write("    'title': None,\n")
        f.write(
            "# 'legend_location': (1, 1) # → グラフ左/下端を0，右/上端を1とした座標に対して，凡例の左上を(1, 1)に指定する\n"
        )
        f.write(
            "# 'legend_location': (1, None) # → グラフ左/下端を0，右/上端を1とした座標に対して，凡例の左をグラフの右端に指定する\n"
        )
        f.write("# 'legend_location': None # → 凡例の場所を指定しない\n")
        f.write("    'legend_location': None,\n")
        f.write(
            "# 'graph_edges': (0.1, 0.2, 0.8, 0.9)\n"
            + "#     → 画面左/下端を0，右/上端を1とした座標に対して，グラフの左，下，右，上端をそれぞれ0.1, 0.2, 0.8, 0.9に指定する\n"
        )
        f.write(
            "# 'graph_edges': (None, None, 0.7, None)\n"
            + "#     → 画面左/下端を0，右/上端を1とした座標に対して，グラフの右端を0.7に指定する\n"
        )
        f.write("# 'graph_edges': None # → グラフの端を指定しない\n")
        f.write("    'graph_edges': None,\n")
        f.write(
            "# 'texts': (('axes', 0.1, 0.8, 'l', r'$\\mathsf{a/b}$', "
            + "{:g}, {:d}),\n".format(plt.rcParams["font.size"], zorder_others)
            + "#           ('data', 0.5, 0.1, 'c', r'exp.', {:g}, {:d}),\n".format(
                plt.rcParams["font.size"], zorder_others
            )
            + "#           ('data', 0.7, 0.9, 'r', r'theory', {:g}, {:d}),)\n".format(
                plt.rcParams["font.size"] + 2.0, zorder_others + 1
            )
            + "#     → axes座標で(0.1, 0.8), data座標で(0.5, 0.1), data座標で(0.7, 0.9)の場所に\n"
            "#       それぞれ左揃え，中央揃え，右揃え，zorder = {:d}, {:d}, {:d}，".format(
            ...
            + "#     * axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標\n"
        )
        f.write("# 'texts': None # 書く文字はない\n")
        f.write(
            "    'texts': (('axes', 0.05, 0.9, 'l', r'test', {:g}, {:d}),),\n".format(
                plt.rcParams["font.size"], zorder_others
            )
        )
        f.write(
            "# 'arrows': (('axes', 0.1, 0.2, 0.4, 0.3, 2.0, '-', 5.0, {:d}),\n".format(
                zorder_others
            )
            + "#            ('data', 0.1, 0.3, 0.4, 0.4, 1.0, '--', 0.0, {:d}),)\n".format(
                zorder_others + 1
            )
            + "#     → axes座標で(0.1, 0.2)から(0.4, 0.3)に線幅2.0，実線，矢の幅5.0，"
            + "zorder = {:d}の矢印，\n".format(zorder_others)
            + "#       data座標で(0.1, 0.3)から(0.4, 0.4)に線幅1.0，破線，矢の幅0.0，"
            + "zorder = {:d}の矢印(=直線)を描く\n".format(zorder_others + 1)
            + "#     * axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標\n"
            + "#     * 線種の指定方法は散布図のline_styleと同じ\n"
        )
        f.write("# 'arrows': None # 描く矢印はない\n")
        f.write(
            "    'arrows': (('axes', 0.1, 0.2, 0.4, 0.3, 2.0, ':', 24.0, {:d}),),\n".format(
                zorder_others
            )
        )
        f.write(
            "# 'ellipses': (('data', 0.1, 0.2, 0.6, 0.4, 20.0, 1.0, True, {:d}),)\n".format(
                zorder_others
            )
            + "#     → data座標で(0.1, 0.2)に中心があり，data座標で幅0.6，高さ0.4で20.0度反時計回りに回転させた線幅1.0，\n"
            + "#       塗りつぶしあり，zorder = {:d}の楕円を描く\n".format(
                zorder_others
            )
            + "#     * axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標\n"
        )
        f.write("# 'ellipses': None # 描く楕円はない\n")
        f.write(
            "    'ellipses': (('axes', 0.8, 0.4, 0.2, 0.1, 45.0, 1.0, False, {:d}),),\n".format(
                zorder_others
            )
        )
        f.write(
            "# 'polygons': (('data', [0.1, 0.1, 0.3, 0.1, 0.2, 0.4], 1.0, False, {:d}),)\n".format(
                zorder_others
            )
            + "#     → data座標で(0.1, 0.1), (0.3, 0.1), (0.2, 0.4)に頂点があり，線幅1.0，塗りつぶしなし，"
            + "zorder = {:d}の多角形を描く\n".format(zorder_others)
            + "#     * axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標\n"
        )
        f.write("# 'polygons': None # 描く多角形はない\n")
        f.write(
            "    'polygons': (('axes', [0.2, 0.2, 0.2, 0.4, 0.3, 0.3], 1.5, True, {:d}),),\n".format(
                zorder_others
            )
        )
        f.write(
            "# 'naca4': (('2412', 'axes', 0.1, 0.1, 0.3, 0.1, 1.0, True, {:d}),)\n".format(
                zorder_others
            )
            + "#     → axes座標で前縁，後縁がそれぞれ(0.1, 0.1), (0.3, 0.1)にあり，線幅1.0，塗りつぶしあり，\n"
            + "#       zorder = {:d}のNACA2412翼を描く\n".format(zorder_others)
            + "#     * axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標\n"
        )
        f.write("# 'naca4': None # 描くNACA4桁系列翼はない\n")
        f.write(
            "    'naca4': (('4415', 'data', -0.6, 0.4, -0.1, 0.1, 1.2, True, {:d}),),\n".format(
                zorder_others
            )
        )
        f.write(
            "# 'param_dict': {'a': 10.0, 'b': 2.2}\n"
            + "#     → ファイル読み取り列指定を数式で行う際に，パラメータa, bをそれぞれ10.0, 2.2として使う\n"
        )
        f.write("# 'param_dict': None # 使うパラメータはない\n")
        f.write("    'param_dict': None,\n")
        f.write("},\n")
        f.write(
            "# '以下の散布図，ベクトル線図，等高線図の辞書（{から}まで）を必要な数だけ作る\n"
        )
        f.write("{\n")
        f.write("# 散布図\n")
        f.write("    'type': 'scatter',\n")
        f.write(
            "# {:d} ≤ zorder ≤ {:d}，zorderが大きいほど前面に描く\n".format(
                zorder_min, zorder_max
            )
        )
        f.write("    'zorder': 10,\n")
        f.write("# データファイル名，このファイルからの相対パス\n")
        f.write("    'file_name': '%s_example1.txt',\n" % s)
        f.write("# <テキストファイルの場合>\n")
        f.write(
            "#  'columns': ('$1/$2*sqrt($3)', '4')\n"
            + "#     → 1列目の値/2列目の値*3列目の値の平方根，4列目の値をそれぞれx, y軸のデータに使う\n"
        )
        f.write(
            "#  'columns': ('1', '2', '5')\n"
            + "#     → 【記号の色や大きさでz軸の値を表したい時】1列目，2列目，5列目の値をそれぞれx, y, z軸のデータに使う\n"
        )
        f.write(
            "# <エクセルファイル（拡張子がxlsまたはxlsx）またはcsvファイルの場合>\n"
        )
        f.write(
            "#  'columns': ('1!A2', 'sqrt(1!B2)')\n"
            + "#     → 1枚目のシートのセルA2から下の値，B2から下の値の平方根をそれぞれx, y軸のデータに使う\n"
        )
        f.write("    'columns': (1, 2),\n")
        f.write(
            "# 'err_columns': ((3, 4), (5, 6))\n"
            + "#     → 3, 4, 5, 6列目の値をそれぞれx方向-, +側，y方向-, +側誤差棒のデータに使う\n"
        )
        f.write(
            "# 'err_columns': (None, 5) # → 5列目の値をy方向-+両側誤差棒のデータに使う\n"
        )
        f.write("# 'err_columns': None # → 誤差棒をつけない\n")
        f.write("    'err_columns': None,\n")
        f.write("# 'every': 2 # → 2行毎にデータを読み込む\n")
        f.write("    'every': 1,\n")
        for i, j in zip(markers[:-1], markers_wx[:-1]):
            f.write("# 'marker': '" + i + "' # → データ点に" + j + "記号をつける\n")
        f.write("# 'marker': None # → データ点に記号をつけない\n")
        f.write("    'marker': 'o',\n")
        f.write("# 'marker_size': None # → 記号の大きさを指定しない\n")
        f.write("# 'marker_size': 8.0 # → 記号の大きさを8.0ポイントにする\n")
        f.write("    'marker_size': None,\n")
        f.write("# 'marker_color': 'white' # → 記号を白で塗りつぶす\n")
        f.write(
            "# 'marker_color': (1.0, 0.1, 0.1) # → 記号を(R, G, B)で指定した色で塗りつぶす，0 ≤ R, G, B ≤ 1\n"
        )
        f.write("    'marker_color': 'white',\n")
        f.write("# 'marker_edge_color': 'red' # → 記号の輪郭の色を赤にする\n")
        f.write("# 'marker_edge_color': 'none' # → 記号の輪郭を付けない\n")
        f.write(
            "# 'marker_edge_color': (1.0, 0.1, 0.1) "
            + "# → 記号の輪郭の色を(R, G, B)で指定した色にする，0 ≤ R, G, B ≤ 1\n"
        )
        f.write("    'marker_edge_color': 'red',\n")
        f.write("# 'line_style': None # → データ点を線でつながない\n")
        for i, j in zip(line_styles[:-1], line_styles_wx[:-1]):
            f.write("# 'line_style': '%s' # → データ点を%sでつなぐ\n" % (i, j))
        f.write("    'line_style': None,\n")
        f.write("# 'line_color': 'red' # → 線の色を赤にする\n")
        f.write(
            "# 'line_color': (1.0, 0.1, 0.1) # → 線の色を(R, G, B)で指定した色にする，0 ≤ R, G, B ≤ 1\n"
        )
        f.write("    'line_color': 'red',\n")
        for i, j in zip(by_what_show_z, by_what_show_z_wx):
            f.write("# 'show_z_by': '%s' # → %sでz軸の値を表す\n" % (i, j))
        f.write("# 'show_z_by': None # → z軸の値はない\n")
        f.write("    'show_z_by': None,\n")
        f.write(
            "# 'marker_size_ratio': 5.0 # → (z軸最大値を表す記号の大きさ)/(z軸最小値を表す記号の大きさ)を5.0倍にする\n"
        )
        f.write(
            "# 'marker_size_ratio': None # → z軸最大値を表す記号の大きさを指定しない\n"
        )
        f.write("    'marker_size_ratio': None,\n")
        f.write(
            "# 'label_in_legend': r'graph for $\\mathsf{a_{xy}^2}$'\n"
            + "#     → 凡例に書くラベル，$\\mathsf{ と }$ の間に書くとTeX形式になる\n"
        )
        f.write("# 'label_in_legend': None # → 凡例に題名を書かない\n")
        f.write("    'label_in_legend': None,\n")
        f.write("},\n")
        f.write("{\n")
        f.write("# ベクトル線図\n")
        f.write("    'type': 'vector',\n")
        ...
        f.write("    'zorder': 2,\n")
        f.write("# ファイル名，このファイルからの相対パス\n")
        f.write("    'file_name': '%s_example2.xlsx',\n" % s)
        f.write("# <テキストファイルの場合>\n")
        f.write(
            "#  'columns': (1, 2, 3, 4) # → 1, 2, 3, 4列目の値をそれぞれx, y軸，u, vのデータに使う\n"
        )
        f.write(
            "#  'columns': ('$1/$2', 'sqrt($3)*10', 3, 4)\n"
            + "#     → 1列目の値/2列目の値，3列目の値の平方根*10, 3, 4列目の値をそれぞれx, y軸，u, vのデータに使う\n"
        )
        f.write(
            "# <エクセルファイル（拡張子がxlsまたはxlsx）またはcsvファイルの場合>\n"
        )
        f.write(
            "#  'columns': ('1!A2', '1!B2', 'sqrt(1!C2)', '1!D2')\n"
            + "#     → 1枚目のシートのセルA2から下の値，B2から下の値，C2から下の値の平方根，D2から下の値をそれぞれx, y軸，\n"
            + "#       u, vのデータに使う\n"
        )
        f.write("    'columns': ('1!A3', '1!B3', '1!D3', '1!E3'),\n")
        f.write("# 'every': 2 # → 2行毎にデータを読み込む\n")
        f.write("    'every': 1,\n")
        f.write("# 'color': 'red' # → 矢印の色を赤にする\n")
        f.write(
            "# 'color': (1.0, 0.1, 0.1) # → 矢印の色を(R, G, B)で指定した色にする，0 ≤ R, G, B ≤ 1\n"
        )
        f.write("# 'color': 'len' # → ベクトルの大きさに応じて矢印の色を虹色で変える\n")
        f.write(
            "# 'color': 'len1' # → ベクトルの大きさに応じて矢印の色を虹色で変え，表示する矢印の長さを全て1にする\n"
        )
        f.write("    'color': 'red',\n")
        f.write("# ベクトルの長さにかける倍率，大きいほど長く表示される\n")
        f.write("    'scale': 1.0,\n")
        f.write(
            "# legend: (0.8, 1.03, 1.0, r'$\\mathsf{u/U_m}$')\n"
            + "#     → 矢印先端の横方向座標を0.8，縦方向座標を1.03として，長さ1.0の矢印で凡例を付ける\n"
            + "#       座標はグラフ左/下端を0，右/上端を1とした座標で指定する\n"
        )
        f.write("# legend: None # → 凡例を付けない\n")
        f.write("    'legend': (0.8, 1.03, 1.0, r'V'),\n")
        f.write("},\n")
        f.write("{\n")
        f.write("# 等高線\n")
        f.write("    'type': 'contour',\n")
        f.write(
            "# {:d} ≤ zorder ≤ {:d}，zorderが大きいほど前面に描く\n".format(
                zorder_min, zorder_max
            )
        ...
        f.write("# ファイル名，このファイルからの相対パス\n")
        f.write("    'file_name': '%s_example2.xlsx',\n" % s)
        f.write("# 'grid_pattern': True # → データが碁盤の目状に並んでいる\n")
        f.write("# 'grid_pattern': False # → データが碁盤の目状に並んでいない\n")
        f.write("    'grid_pattern': False,\n")
        f.write(
            "# 'show_triangle': True # → データが碁盤の目状に並んでいない時に行う，デローニー分割した三角形を表示する\n"
        )
        f.write(
            "# 'show_triangle': False # → データが碁盤の目状に並んでいない時に行う，デローニー分割した三角形を表示しない\n"
        )
        f.write("    'show_triangle': False,\n")
        f.write("# <テキストファイルの場合>\n")
        f.write(
            "#  'columns': (1, 2, 3) # → 1, 2, 3列目の値をそれぞれx, y, z軸のデータに使う\n"
        )
        f.write(
            "#  'columns': ('$1/$2', 'sqrt($3)*10', 3)\n"
            + "#     → 1列目の値/2列目の値，3列目の値の平方根*10, 3列目の値をそれぞれx, y, z軸のデータに使う\n"
        )
        f.write(
            "# <エクセルファイル（拡張子がxlsまたはxlsx）またはcsvファイルの場合>\n"
        )
        f.write(
            "#  'columns': ('1!A2', '1!B2', 'sqrt(1!C2)')\n"
            "#     → 1枚目のシートのセルA2から下の値，B2から下の値，C2から下の値の平方根をそれぞれx, y, z軸のデータに使う\n"
        )
        f.write("    'columns': ('1!A3', '1!B3', '1!C3'),\n")
        f.write("# 'every': 2 # → 2行毎にデータを読み込む\n")
        f.write("    'every': 1,\n")
        for i, j in zip(paint_styles[:-1], paint_styles_wx[:-1]):
            f.write("# 'paint': '%s' # → %sで塗る\n" % (i, j))
        f.write("# 'paint': None # → %s\n" % paint_styles_wx[-1])
        f.write("    'paint': 'rainbow',\n")
        f.write("# 'smooth_paint': True # → 面塗りを滑らかにする\n")
        f.write("    'smooth_paint': True,\n")
        f.write("},\n")
    s = os.path.splitext(file_name)[0]  # unicode
    with codecs.open("%s_example1.txt" % s, "w", encoding="UTF-8") as f:
        f.write("#y = x\n")
        f.write("#x\ty\n")
...
tooltip_tex = (
    _(
        "$\\mathsf{ と }$ の間に書くとTeX形式になり，上付き，下付き文字やギリシア文字が使えます．\n例）"
    )
...
class TableForGraph(MyTable):
    def __init__(self):
        ...
        self.ROW_Y = 1
        self.ROW_Z = 2
        self.row_labels = (_("x軸"), _("y軸"), _("z軸"))
        ...
        s1 = _("空白の場合，自動スケールになります．")
        s2 = _("等高線またはベクトルの大きさを表す色に対する軸\n")
        self.tooltips = (
            (s1, None, s1, s1, None, tooltip_tex),
            (s1, None, s1, s1, None, tooltip_tex),
        ...
        self.data = [
            ...
            [None, "〜", None, None, False, "y"],
            [None, "〜", None, None, False, "z"],
        ]
...
tooltip_zorder = ("{:d} ≤ Z-order ≤ {:d}\n".format(zorder_min, zorder_max)) + _(
    "大きいほど前面に描きます．"
)
tooltip_coordinate = _(
    "axes座標はグラフ枠の左下を(0, 0), 右上を(1, 1)にとる座標，data座標はふつうのx, y座標"
)
...
class TableForText(MyTable):
    def __init__(self):
        ...
        self.tooltips = (
            (tooltip_coordinate, None, None, None, tooltip_tex, None, tooltip_zorder),
        )
        self.data_types = (
            wx.grid.GRID_VALUE_CHOICE + ":axes,data",
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_CHOICE + _(":左,中央,右"),
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_STRING,
    ...
    def GetValue(self, row, col):
        if col in (self.COL_COORDINATE, self.COL_ZORDER):
            ...
            return (
                _("左")
                if self.data[row][col] == "l"
                else (_("中央") if self.data[row][col] == "c" else _("右"))
            )
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_COORDINATE:
            ...
            self.data[row][col] = (
                "l" if value == _("左") else ("c" if value == _("中央") else "r")
            )
        elif col == self.COL_STRING:
            value = (
...
class TableForArrow(MyTable):
    def __init__(self):
        ...
        self.data_types = (
            ...
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_CHOICE + ":" + ",".join(line_styles_wx[:-1]),
            wx.grid.GRID_VALUE_STRING,
            wx.grid.GRID_VALUE_NUMBER + ":{:d},{:d}".format(zorder_min, zorder_max),
        )
        self.new_data = [
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_COORDINATE:
            ...
            try:
                ...
                self.data[row][col] = (
                    self.new_data[self.COL_LINE_WIDTH]
                    if col == self.COL_LINE_WIDTH
                    else (
                        self.new_data[self.COL_HEAD_WIDTH]
                        if col == self.COL_HEAD_WIDTH
                        else None
                    )
                )
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_COORDINATE:
            ...
            try:
                ...
                self.data[row][col] = (
                    self.new_data[self.COL_ANGLE]
                    if col == self.COL_ANGLE
                    else (
                        self.new_data[self.COL_LINE_WIDTH]
                        if col == self.COL_LINE_WIDTH
                        else None
    ...
    def __init__(self):
        ...
        self.tooltips = (
            (
                tooltip_coordinate,
                _(
                    "1点目の横座標, 1点目の縦座標, 2点目の横座標, 2点目の縦座標, ...\nのようにコンマ区切りで記入"
                ),
                None,
                None,
                tooltip_zorder,
        ...
        if col in (self.COL_COORDINATE, self.COL_ZORDER):
            ...
            return (
                ...
                if self.data[row][col] is None
                    ...
                    ["{:g}, {:g}".format(i[0], i[1]) for i in self.data[row][col]]
                )
            )
        elif col == self.COL_FILL:
            return "1" if self.data[row][col] else ""
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_COORDINATE:
            ...
            try:
                ...
                self.data[row][col] = [
                    [value[i], value[i + 1]] for i in range(0, len(value), 2)
                ]
            except:
                self.data[row][col] = None
        elif col == self.COL_FILL:
...
class TableForNaca4(MyTable):
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_4DIGITS:
            try:
                self.data[row][col] = ("%04d" % int(value))[:4]
            except:
                self.data[row][col] = None
...
class TableForParamDict(MyTable):
    def __init__(self):
        ...
        self.COL_SYMBOL = 0
        self.COL_VALUE = 1
        self.col_labels = (_("パラメータ"), _("数値"))
        self.col_sizes = (200, 300)
        self.tooltips = ((_("数字で始まるパラメータは不可"), None),)
    ...
    def SetValue(self, row, col, value):
        if col == self.COL_SYMBOL:
            ...
            self.data[row][col] = None if value == "" or value[0].isdigit() else value
        else:
            try:
                self.data[row][col] = float(value)
            except:
...
class GridWithCellToolTip(wx.grid.Grid):
    def __init__(
        ...
        self.EnableDragRowSize(
            False
        )  # needed to avoid "Segmentation fault: 11" in button_delete_gridOnButtonClick
    ...
    def OnMouseOver(self, event):
        ...
        try:
            if self.table.tooltips is not None:
                ...
                c = self.XYToCell(
                    *self.CalcUnscrolledPosition(event.GetX(), event.GetY())
                )
    ...
    def UpdateView(self, numOfRowsIncreased):
        ...
        self.ProcessTableMessage(
            wx.grid.GridTableMessage(
                self.table, wx.grid.GRIDTABLE_NOTIFY_ROWS_APPENDED, numOfRowsIncreased
            )
        )
        self.ProcessTableMessage(
            wx.grid.GridTableMessage(
                self.table, wx.grid.GRIDTABLE_REQUEST_VIEW_GET_VALUES
            )
        )
        self.EndBatch()
...
class FrameMain(wx.Frame):
    def __init__(self, parent):
        ...
        tooltip_either_empty = _("どちらかが空白の場合，指定しません．")
        tooltip_graph_coordinate = _(
            "空白の場合，指定しません．\nグラフ左/下端を0，右/上端を1とします．"
        )
        tooltip_screen_coordinate = _(
            "空白の場合，指定しません．\n画面左/下端を0，右/上を1とします．"
        )
        tooltip_append_row = _("下に行を追加")
        tooltip_delete_row = _("選択している行を削除")
        label_reset = _("リセット")
        tooltip_column = (
            _(
                "<テキストファイルの場合>\n"
            ...
            + _(
                "<エクセルファイル（拡張子がxlsまたはxlsx）またはcsvファイルの場合>\n"
                + " 'シート番号!読み取り開始のセル番号' で指定します．\n"
                + " 行方向にデータがあるものとします．\n"
                + " テキストファイルの場合と同様に，数式が使えます．\n 例）"
            )
            + "'sqrt(1!A2)'"
        )
        tooltip_err_column = _(
            "両側が空白の場合，指定しません．\nいずれかだけの場合，-+両側で同じデータを使用します．"
        )
        tooltip_drag_and_drop = _("ドラッグ&ドロップで決めることもできます．")
        tooltip_equation_scatter = (
            _(
                "次の書式を用いた数式のプロットも可能です．\n"
                + "分割数の前の'/'を'L/'にすれば，対数軸上で等分割にできます：\n"
                + " 数式, 変数 = [最小値, 最大値]/分割数\n [xの数式, yの数式], 変数 = [最小値, 最大値]/分割数\n例）"
            )
            + "sqrt(2.2*x), x = [0.0, 1.0]/100\n        [cos(t), sin(t)], t = [0.0, pi]/100"
        )
        tooltip_open_file = _("ファイルを開く")

        spinCtrlWidth = 118 if sys.platform == "linux2" else 60

        bSizer1 = wx.BoxSizer(wx.VERTICAL)

        self.scrolledWindow_graph = wx.ScrolledWindow(
        ...
        self.filePicker_setting = wx.FilePickerCtrl(
            self.scrolledWindow_graph,
            wx.ID_ANY,
            wx.EmptyString,
            _("設定ファイルを開く"),
            ...
            name="filePicker_setting",
        )
        self.filePicker_setting.SetForegroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
        )
        self.filePicker_setting.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
        )
        self.filePicker_setting.SetToolTip(tooltip_drag_and_drop)
        self.filePicker_setting.SetDropTarget(
            FileDropTargetForSetting(self.filePicker_setting, self)
        )
        bSizer3.Add(
            self.filePicker_setting, 1, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM, 5
        )

        self.button_setting_open_file = wx.Button(
            self.scrolledWindow_graph,
            wx.ID_ANY,
            _("開く"),
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="bfilePicker_setting",
        )
        self.button_setting_open_file.SetToolTip(_("設定ファイルを開く"))
        bSizer3.Add(
            self.button_setting_open_file, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )

        bSizer2.Add(bSizer3, 0, wx.EXPAND, 5)

        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("画像")), wx.HORIZONTAL
        )
        ...
        staticText1.Wrap(-1)
        sbSizer1.Add(
            staticText1, 0, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.LEFT, 5
        )

        self.filePicker_png = wx.FilePickerCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            _("pngファイルを保存"),
        ...
        self.filePicker_png.SetForegroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
        )
        self.filePicker_png.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
        )
        self.filePicker_png.SetToolTip(
            _("空白の場合，matplotlibのビューワーにグラフを表示します．\n")
            + tooltip_drag_and_drop
        )
        self.filePicker_png.SetDropTarget(MyFileDropTarget(self.filePicker_png))
        sbSizer1.Add(
            self.filePicker_png, 1, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM, 5
        )
        ...
        self.textCtrl_fig_size_w = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_fig_size_h = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        staticText1.Wrap(-1)
        sbSizer1.Add(
            staticText1, 0, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT, 5
        )

        bSizer2.Add(sbSizer1, 0, wx.EXPAND, 5)

        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("データファイル")),
            wx.HORIZONTAL,
        )
        ...
        self.textCtrl_skip = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "#",
            wx.DefaultPosition,
        ...
        self.textCtrl_delimiter = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_delimiter.SetToolTip(
            _("空白の場合，半角スペースかタブ区切りになります．")
        )
        sbSizer1.Add(
            self.textCtrl_delimiter,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        self.textCtrl_terminator = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "!!STOP",
            wx.DefaultPosition,
        ...
        self.textCtrl_terminator.SetToolTip(_("空白の場合，最後の行まで読み込みます．"))
        sbSizer1.Add(
            self.textCtrl_terminator,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("グラフ")), wx.VERTICAL
        )

        self.grid_graph = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.checkBox_x_grid = wx.CheckBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("x軸目盛線"),
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.checkBox_x_grid,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.LEFT,
            5,
        )

        self.checkBox_y_grid = wx.CheckBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("y軸目盛線"),
            wx.DefaultPosition,
        ...
        self.comboBox_aspect = wx.ComboBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.comboBox_aspect.SetValue("auto")
        self.comboBox_aspect.SetForegroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
        )
        self.comboBox_aspect.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
        )
        self.comboBox_aspect.SetToolTip(
            _(
                "数値またはauto, equal\n数値は(y軸の長さ)/(x軸の長さ)*(x軸の最大値-最小値)/(y軸の最大値-最小値)の値を入力します．\nequalは1と同じ意味です．"
            )
        )
        bSizer3.Add(
            self.comboBox_aspect,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        self.textCtrl_title = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_legend_left = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_legend_top = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_graph_left = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_graph_bottom = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_graph_right = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.textCtrl_graph_top = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("文字列")), wx.VERTICAL
        )

        self.grid_text = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.button_append_text = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
        ...
        self.button_delete_text = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
        ...
        self.button_clear_text = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(
                self.scrolledWindow_graph,
                wx.ID_ANY,
                _("矢印．矢の幅を0にすれば直線になります．"),
            ),
            wx.VERTICAL,
        )

        self.grid_arrow = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.button_append_arrow = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
        ...
        self.button_delete_arrow = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
        ...
        self.button_clear_arrow = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("楕円")), wx.VERTICAL
        )

        self.grid_ellipse = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.button_append_ellipse = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
        ...
        self.button_delete_ellipse = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
        ...
        self.button_clear_ellipse = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("多角形")), wx.VERTICAL
        )

        self.grid_polygon = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.button_append_polygon = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
        ...
        self.button_delete_polygon = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
        ...
        self.button_clear_polygon = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("NACA 4桁系列翼")),
            wx.VERTICAL,
        )

        self.grid_naca4 = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        self.button_append_naca4 = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
        ...
        self.button_delete_naca4 = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
        ...
        self.button_clear_naca4 = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(
                self.scrolledWindow_graph,
                wx.ID_ANY,
                _("ファイル読み取り列指定を数式で行う際に使うパラメータ"),
            ),
            wx.VERTICAL,
        )

        self.grid_param_dict = GridWithCellToolTip(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="grid_param_dict",
            table=TableForParamDict(),
        )
        ...
        self.button_append_param_dict = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "+",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="agrid_param_dict",
        )
        self.button_append_param_dict.SetToolTip(tooltip_append_row)
        bSizer3.Add(
            self.button_append_param_dict, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )

        self.button_delete_param_dict = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "-",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="dgrid_param_dict",
        )
        self.button_delete_param_dict.SetToolTip(tooltip_delete_row)
        bSizer3.Add(
            self.button_delete_param_dict, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )

        self.button_clear_param_dict = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            label_reset,
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="cgrid_param_dict",
        )
        bSizer3.Add(
            self.button_clear_param_dict, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )
        ...
        for i in range(n_scatter):
            ...
            self.spinCtrls_zorder_scatter.append(
                wx.SpinCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.EmptyString,
                    wx.DefaultPosition,
                    wx.Size(spinCtrlWidth, -1),
                    wx.SP_ARROW_KEYS,
                    zorder_min,
                    zorder_max,
                    zorder_scatter + i,
            ...
            bSizer3.Add(
                self.spinCtrls_zorder_scatter[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.filePickers_scatter.append(
                wx.FilePickerCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.EmptyString,
                    ...
                    | wx.FLP_SMALL,
                    name="filePicker_scatter{:d}".format(i),
                )
            )
            self.filePickers_scatter[i].SetForegroundColour(
                wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
            )
            self.filePickers_scatter[i].SetBackgroundColour(
                wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
            )
            self.filePickers_scatter[i].SetToolTip(
                tooltip_drag_and_drop + "\n\n" + tooltip_equation_scatter
            )
            self.filePickers_scatter[i].SetDropTarget(
                MyFileDropTarget(self.filePickers_scatter[i])
            )
            bSizer3.Add(
                self.filePickers_scatter[i],
                1,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )

            self.buttons_scatter_open_file.append(
                wx.Button(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    _("開く"),
                    ...
                    wx.DefaultSize,
                    0,
                    name="bfilePicker_scatter{:d}".format(i),
                )
            )
            self.buttons_scatter_open_file[i].SetToolTip(tooltip_open_file)
            bSizer3.Add(
                self.buttons_scatter_open_file[i],
                0,
            ...
            self.textCtrls_scatter_column_x.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "1",
            ...
            bSizer3.Add(
                self.textCtrls_scatter_column_x[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_column_y.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "2",
            ...
            bSizer3.Add(
                self.textCtrls_scatter_column_y[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_err_column_xn.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "",
            ...
            self.textCtrls_scatter_err_column_xn[i].SetToolTip(
                tooltip_err_column + "\n\n" + tooltip_column
            )
            bSizer3.Add(
                self.textCtrls_scatter_err_column_xn[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_err_column_xp.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "",
            ...
            self.textCtrls_scatter_err_column_xp[i].SetToolTip(
                tooltip_err_column + "\n\n" + tooltip_column
            )
            bSizer3.Add(
                self.textCtrls_scatter_err_column_xp[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_err_column_yn.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "",
            ...
            self.textCtrls_scatter_err_column_yn[i].SetToolTip(
                tooltip_err_column + "\n\n" + tooltip_column
            )
            bSizer3.Add(
                self.textCtrls_scatter_err_column_yn[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_err_column_yp.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "",
            ...
            self.textCtrls_scatter_err_column_yp[i].SetToolTip(
                tooltip_err_column + "\n\n" + tooltip_column
            )
            bSizer3.Add(
                self.textCtrls_scatter_err_column_yp[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_every.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "1",
            ...
            bSizer3.Add(
                self.textCtrls_scatter_every[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            staticText1.Wrap(-1)
            bSizer3.Add(
                staticText1, 0, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM, 5
            )

            self.textCtrls_scatter_label.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.EmptyString,
            ...
            bSizer3.Add(
                self.textCtrls_scatter_label[i],
                1,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
                5,
            )
            ...
            self.choices_scatter_marker.append(
                wx.Choice(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.DefaultPosition,
            ...
            self.textCtrls_scatter_marker_size.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.EmptyString,
            ...
            self.textCtrls_scatter_marker_size[i].SetToolTip(
                _("空白の場合，指定しません．")
            )
            self.textCtrls_scatter_marker_size[i].SetMinSize(wx.Size(60, -1))
            bSizer3.Add(
                self.textCtrls_scatter_marker_size[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.colourPickers_scatter_marker.append(
                wx.ColourPickerCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
            ...
            bSizer3.Add(
                self.colourPickers_scatter_marker[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )

            self.checkBoxes_scatter_marker_edge.append(
                wx.CheckBox(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    _("記号の輪郭色："),
            ...
            self.checkBoxes_scatter_marker_edge[i].SetToolTip(
                _("チェックを外すと輪郭を付けません．")
            )
            self.checkBoxes_scatter_marker_edge[i].SetValue(True)
            bSizer3.Add(
                self.checkBoxes_scatter_marker_edge[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )

            self.colourPickers_scatter_marker_edge.append(
                wx.ColourPickerCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.Colour(255, 0, 0),
            ...
            bSizer3.Add(
                self.colourPickers_scatter_marker_edge[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
                5,
            )
            ...
            self.choices_scatter_line_style.append(
                wx.Choice(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.DefaultPosition,
            ...
            bSizer3.Add(
                self.choices_scatter_line_style[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.colourPickers_scatter_line.append(
                wx.ColourPickerCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.Colour(255, 0, 0),
            ...
            bSizer3.Add(
                self.colourPickers_scatter_line[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
                5,
            )
            ...
            self.textCtrls_scatter_column_z.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    "",
            ...
            self.textCtrls_scatter_column_z[i].SetToolTip(
                _("空白の場合，普通の散布図になります．\n") + tooltip_column
            )
            bSizer3.Add(
                self.textCtrls_scatter_column_z[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
            ...
            self.textCtrls_scatter_marker_size_ratio.append(
                wx.TextCtrl(
                    sbSizer1.GetStaticBox(),
                    wx.ID_ANY,
                    wx.EmptyString,
            ...
            bSizer3.Add(
                self.textCtrls_scatter_marker_size_ratio[i],
                0,
                wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
                5,
            )
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("ベクトル線図")),
            wx.VERTICAL,
        )
        ...
        self.spinCtrl_zorder_vector = wx.SpinCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.filePicker_vector = wx.FilePickerCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            _("ファイルを開く"),
            ...
            wx.DefaultSize,
            wx.FLP_USE_TEXTCTRL | wx.FLP_OPEN | wx.FLP_FILE_MUST_EXIST | wx.FLP_SMALL,
            name="filePicker_vector",
        )
        self.filePicker_vector.SetForegroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
        )
        self.filePicker_vector.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
        )
        self.filePicker_vector.SetToolTip(
            tooltip_drag_and_drop
            + "\n\n"
            + _(
                "次の書式を用いた数式のプロットも可能です．\n"
                + "分割数の前の'/'を'L/'にすれば，対数軸上で等分割にできます：\n"
                + " [uの数式, vの数式], xの変数 = [最小値, 最大値]/分割数, yの変数 = [最小値, 最大値]/分割数\n例）"
            )
            + "[cos(2.0*x), sin(y)], x = [-1.0, 1.0]/100, y = [-1.00, 1.0]/100"
        )
        self.filePicker_vector.SetDropTarget(MyFileDropTarget(self.filePicker_vector))
        bSizer3.Add(
            self.filePicker_vector, 1, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM, 5
        )

        self.button_vector_open_file = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("開く"),
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="bfilePicker_vector",
        )
        self.button_vector_open_file.SetToolTip(tooltip_open_file)
        bSizer3.Add(
            self.button_vector_open_file, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )
        ...
        self.textCtrl_vector_column_x = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_column_y = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "2",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_column_u = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "3",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_column_v = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "4",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_every = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1",
            wx.DefaultPosition,
        ...
        self.colourPicker_vector = wx.ColourPickerCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.Colour(255, 0, 0),
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.colourPicker_vector,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )

        self.choice_vector_color = wx.Choice(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
            [_("虹色なし"), _("虹色あり"), _("虹色+長さ1")],
            0,
        )
        self.choice_vector_color.SetSelection(0)
        self.choice_vector_color.SetToolTip(
            _(
                "「虹色なし」は単色指定．\n"
                + "「虹色あり」はベクトルの大きさに応じて矢印の色を虹色で変える．\n"
                + "「虹色+長さ1」はベクトルの大きさに応じて矢印の色を虹色で変え，表示する矢印の長さを全て1にする．"
            )
        )
        bSizer3.Add(
            self.choice_vector_color,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_vector_scale = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1.0",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_scale.SetToolTip(
            _("ベクトルの長さにかける倍率．大きいほど長く表示される．")
        )
        self.textCtrl_vector_scale.SetMinSize(wx.Size(60, -1))
        bSizer3.Add(
            self.textCtrl_vector_scale,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        staticText1 = wx.StaticText(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            (_("!!Python 3でのみ有効!!→") if sys.version_info.major <= 2 else "")
            + _("凡例の横座標："),
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        ...
        self.textCtrl_vector_legend_x = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "0.8",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_legend_x.SetToolTip(
            _("凡例矢印先端の横座標，0/1だとグラフ枠の左/右端")
        )
        self.textCtrl_vector_legend_x.SetMinSize(wx.Size(60, -1))
        if sys.version_info.major <= 2:
            self.textCtrl_vector_legend_x.Disable()
        bSizer3.Add(
            self.textCtrl_vector_legend_x,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_vector_legend_y = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1.03",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_legend_y.SetToolTip(
            _("凡例矢印先端の縦座標，0/1だとグラフ枠の下/上端")
        )
        self.textCtrl_vector_legend_y.SetMinSize(wx.Size(60, -1))
        if sys.version_info.major <= 2:
            self.textCtrl_vector_legend_y.Disable()
        bSizer3.Add(
            self.textCtrl_vector_legend_y,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_vector_legend_u = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1.0",
            wx.DefaultPosition,
        ...
        self.textCtrl_vector_legend_label = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.textCtrl_vector_legend_label,
            1,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        sbSizer1 = wx.StaticBoxSizer(
            wx.StaticBox(self.scrolledWindow_graph, wx.ID_ANY, _("等高線")), wx.VERTICAL
        )
        ...
        self.spinCtrl_zorder_contour = wx.SpinCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            wx.DefaultPosition,
        ...
        self.filePicker_contour = wx.FilePickerCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.EmptyString,
            _("ファイルを開く"),
            ...
            wx.DefaultSize,
            wx.FLP_USE_TEXTCTRL | wx.FLP_OPEN | wx.FLP_FILE_MUST_EXIST | wx.FLP_SMALL,
            name="filePicker_contour",
        )
        self.filePicker_contour.SetForegroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)
        )
        self.filePicker_contour.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
        )
        self.filePicker_contour.SetToolTip(
            tooltip_drag_and_drop
            + "\n\n"
            + _(
                "次の書式を用いた数式のプロットも可能です．\n"
                + "分割数の前の'/'を'L/'にすれば，対数軸上で等分割にできます：\n"
                + " 数式, xの変数 = [最小値, 最大値]/分割数, yの変数 = [最小値, 最大値]/分割数\n例）"
            )
            + "sqrt(x)*y**2, x = [-1.0, 1.0]/100, y = [-1.0, 1.0]/100"
        )
        self.filePicker_vector.SetDropTarget(MyFileDropTarget(self.filePicker_vector))
        ...
        bSizer3.Add(
            self.filePicker_contour, 1, wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM, 5
        )

        self.button_contour_open_file = wx.Button(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("開く"),
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
            name="bfilePicker_contour",
        )
        self.button_contour_open_file.SetToolTip(tooltip_open_file)
        bSizer3.Add(
            self.button_contour_open_file, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )
        ...
        self.checkBox_contour_grid_pattern = wx.CheckBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("碁盤の目状"),
            wx.DefaultPosition,
        ...
        self.checkBox_contour_grid_pattern.SetToolTip(
            _(
                "データが碁盤の目状に並んでいる時にチェックを付けると，描画が高速になります．"
            )
        )
        self.checkBox_contour_grid_pattern.SetValue(False)
        bSizer3.Add(
            self.checkBox_contour_grid_pattern, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )

        self.checkBox_contour_show_triangle = wx.CheckBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("三角形を表示"),
            wx.DefaultPosition,
        ...
        self.checkBox_contour_show_triangle.SetToolTip(
            _(
                "「碁盤の目状」にチェックがついていない時に行う，デローニー分割した三角形を表示します．"
            )
        )
        self.checkBox_contour_show_triangle.SetValue(False)
        bSizer3.Add(
            self.checkBox_contour_show_triangle, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5
        )
        ...
        self.textCtrl_contour_column_x = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1",
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.textCtrl_contour_column_x,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_contour_column_y = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "2",
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.textCtrl_contour_column_y,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_contour_column_z = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "3",
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.textCtrl_contour_column_z,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM,
            5,
        )
        ...
        self.textCtrl_contour_every = wx.TextCtrl(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            "1",
            wx.DefaultPosition,
        ...
        self.choice_contour_paint = wx.Choice(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            wx.DefaultPosition,
            wx.DefaultSize,
        ...
        bSizer3.Add(
            self.choice_contour_paint,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )

        self.checkBox_contour_smooth_paint = wx.CheckBox(
            sbSizer1.GetStaticBox(),
            wx.ID_ANY,
            _("滑らかな面塗り"),
            wx.DefaultPosition,
        ...
        bSizer3.Add(
            self.checkBox_contour_smooth_paint,
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.TOP | wx.BOTTOM | wx.RIGHT,
            5,
        )
        ...
        self.filePickers = self.filePickers_scatter + [
            self.filePicker_png,
            self.filePicker_vector,
            self.filePicker_contour,
        ]
        ...
        self.menuItem_open = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("開く") + "\tCtrl+O",
            wx.EmptyString,
        ...
        self.menuItem_save = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("保存") + "\tCtrl+S",
            wx.EmptyString,
        ...
        self.menuItem_close = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("閉じる") + "\tCtrl+W",
            wx.EmptyString,
        ...
        self.menuItem_save_as = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("別名で保存") + "\tShift+Ctrl+S",
            wx.EmptyString,
            wx.ITEM_NORMAL,
        )
        self.menu_file.Append(self.menuItem_save_as)
        self.menu_file.AppendSeparator()
        self.menuItem_save_as_script = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("pythonスクリプト形式で保存") + "\tCtrl+K",
            wx.EmptyString,
            wx.ITEM_NORMAL,
        )
        self.menu_file.Append(self.menuItem_save_as_script)
        self.menu_file.AppendSeparator()
        self.menuItem_make_template = wx.MenuItem(
            self.menu_file,
            wx.ID_ANY,
            _("テンプレートを作る") + "\tCtrl+T",
            wx.EmptyString,
        ...
        self.menubar.Append(self.menu_file, _("設定ファイル") + "(&F)")

        self.menu_edit = wx.Menu()
        self.menuItem_cut = wx.MenuItem(
            self.menu_edit,
            wx.ID_ANY,
            _("カット") + "\tCtrl+X",
            wx.EmptyString,
        ...
        self.menuItem_copy = wx.MenuItem(
            self.menu_edit,
            wx.ID_ANY,
            _("コピー") + "\tCtrl+C",
            wx.EmptyString,
        ...
        self.menuItem_paste = wx.MenuItem(
            self.menu_edit,
            wx.ID_ANY,
            _("ペースト") + "\tCtrl+V",
            wx.EmptyString,
        ...
        self.menubar.Append(self.menu_edit, _("編集") + "(&E)")

        self.menu_plot = wx.Menu()
        self.menuItem_plot = wx.MenuItem(
            self.menu_plot,
            wx.ID_ANY,
            _("グラフをプロット") + "\tCtrl+P",
            wx.EmptyString,
        ...
        self.menuItem_plot_direct = wx.MenuItem(
            self.menu_plot,
            wx.ID_ANY,
            _("設定ファイルから直接プロット") + "\tShift+Ctrl+P",
            wx.EmptyString,
            wx.ITEM_NORMAL,
        )
        self.menu_plot.Append(self.menuItem_plot_direct)
        self.menubar.Append(self.menu_plot, _("プロット") + "(&P)")

        self.menu_help = wx.Menu()
        self.menuItem_update = wx.MenuItem(
            self.menu_help, wx.ID_ANY, _("アップデート"), wx.EmptyString, wx.ITEM_NORMAL
        )
        self.menu_help.Append(self.menuItem_update)
        self.menuItem_movie = wx.MenuItem(
            self.menu_help, wx.ID_ANY, _("使い方の動画"), wx.EmptyString, wx.ITEM_NORMAL
        )
        self.menu_help.Append(self.menuItem_movie)
        self.menubar.Append(self.menu_help, _("ヘルプ") + "(&H)")
        ...
        self.button_append_text.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_text.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        self.button_clear_text.Bind(wx.EVT_BUTTON, self.button_clear_gridOnButtonClick)
        self.button_append_param_dict.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_param_dict.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        self.button_clear_param_dict.Bind(
            wx.EVT_BUTTON, self.button_clear_gridOnButtonClick
        )
        self.button_append_arrow.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_arrow.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        self.button_clear_arrow.Bind(wx.EVT_BUTTON, self.button_clear_gridOnButtonClick)
        self.button_append_ellipse.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_ellipse.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        self.button_clear_ellipse.Bind(
            wx.EVT_BUTTON, self.button_clear_gridOnButtonClick
        )
        self.button_append_polygon.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_polygon.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        self.button_clear_polygon.Bind(
            wx.EVT_BUTTON, self.button_clear_gridOnButtonClick
        )
        self.button_append_naca4.Bind(
            wx.EVT_BUTTON, self.button_append_gridOnButtonClick
        )
        self.button_delete_naca4.Bind(
            wx.EVT_BUTTON, self.button_delete_gridOnButtonClick
        )
        ...
        self.checkBox_contour_grid_pattern.Bind(
            wx.EVT_CHECKBOX, self.checkBox_contour_grid_patternOnCheck
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_openOnMenuSelection,
            id=self.menuItem_open.GetId(),
        )
        self.Bind(wx.EVT_MENU, self.FrameMainOnClose, id=self.menuItem_close.GetId())
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_saveOnMenuSelection,
            id=self.menuItem_save.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_save_asOnMenuSelection,
            id=self.menuItem_save_as.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_save_as_scriptOnMenuSelection,
            id=self.menuItem_save_as_script.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_make_templateOnMenuSelection,
            id=self.menuItem_make_template.GetId(),
        )
        self.Bind(
            wx.EVT_MENU, self.menuItem_cutOnMenuSelection, id=self.menuItem_cut.GetId()
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_copyOnMenuSelection,
            id=self.menuItem_copy.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_pasteOnMenuSelection,
            id=self.menuItem_paste.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_plotOnMenuSelection,
            id=self.menuItem_plot.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_plot_directOnMenuSelection,
            id=self.menuItem_plot_direct.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_updateOnMenuSelection,
            id=self.menuItem_update.GetId(),
        )
        self.Bind(
            wx.EVT_MENU,
            self.menuItem_movieOnMenuSelection,
            id=self.menuItem_movie.GetId(),
        )

        backup_path_old = correct_file_name_in_unicode(
            os.path.join(
                os.path.dirname(os.path.realpath(decode_if_necessary(__file__))),
                "backup_matplotlib.txt",
            )
        )  # unicode
        self.backup_path = correct_file_name_in_unicode(
            os.path.join(
                os.path.dirname(os.path.realpath(decode_if_necessary(__file__))),
                "backup_matplotlibwx.txt",
            )
        )  # unicode
        ...
        self.cwd = correct_file_name_in_unicode(
            decode_if_necessary(os.getcwd())
        )  # unicode
    ...
    def FrameMainOnClose(self, event):
        with wx.MessageDialog(
            self,
            _("設定を保存しますか？")
            if self.filePicker_setting.GetPath() == ""
            else _("設定を ")
            + self.filePicker_setting.GetPath()
            + _(" に保存しますか？"),
            _("保存"),
    ...
    def OnFileChanged(self, event):
        ...
        if p == "" or pat_eq_plot.match(p):
            event.GetEventObject().GetTextCtrl().SetBackgroundColour(
                wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
            )
    ...
    def button_openOnButtonClick(self, event):
        p = self.FindWindowByName(
            event.GetEventObject().GetName()[1:], event.GetEventObject().GetParent()
        ).GetPath()  # unicode
        if p == "":
            return
        if p.endswith(".zip"):
        ...
        if sys.platform == "win32":
            ...
            subprocess.call('open "{}"'.format(p), shell=True)
        else:
            subprocess.call('xdg-open "{}"'.format(p), shell=True)
    ...
    def button_increase_gridOnButtonClick(self, event):
        ...
        if row != len(grid.table.data) - 1:
            grid.table.data[row], grid.table.data[row + 1] = (
                grid.table.data[row + 1],
                grid.table.data[row],
            )
    ...
    def load_settings(self, path):
        ...
        try:
            self.filePicker_png.SetPath(
                correct_file_name_in_unicode(
                    os.path.join(dir_name, s[0]["png_file_name"])
                )
                if "png_file_name" in s[0] and s[0]["png_file_name"] is not None
                else ""
            )
        except:
            print(sys.exc_info())
        for i, j in ((self.textCtrl_fig_size_w, 0), (self.textCtrl_fig_size_h, 1)):
            try:
                i.SetValue(
                    "{:g}".format(s[0]["fig_size"][j])
                    if "fig_size" in s[0]
                    and s[0]["fig_size"] is not None
                    and s[0]["fig_size"][j] is not None
                    else ""
                )
            except:
                print(sys.exc_info())
        for i, j in (
            (self.textCtrl_skip, "skip"),
            (self.textCtrl_delimiter, "delimiter"),
            (self.textCtrl_terminator, "terminator"),
            (self.textCtrl_title, "title"),
        ):
            try:
                i.SetValue(s[0][j] if j in s[0] and s[0][j] is not None else "")
            except:
                print(sys.exc_info())
        for i, j, k, l in (
            (self.grid_graph.table.ROW_X, self.grid_graph.table.COL_FROM, 0, 0),
            ...
            try:
                self.grid_graph.table.data[i][j] = (
                    s[0]["ranges"][k][l]
                    if "ranges" in s[0]
                    and s[0]["ranges"] is not None
                    and s[0]["ranges"][k] is not None
                    else None
                )
            except:
                print(sys.exc_info())
        for i, j in (
            (self.grid_graph.table.ROW_X, 0),
            (self.grid_graph.table.ROW_Y, 1),
            (self.grid_graph.table.ROW_Z, 2),
        ):
            try:
                self.grid_graph.table.data[i][self.grid_graph.table.COL_TICKS] = (
                    s[0]["ticks"][j]
                    if "ticks" in s[0] and s[0]["ticks"] is not None
                    else None
                )
            except:
                print(sys.exc_info())
            try:
                self.grid_graph.table.data[i][self.grid_graph.table.COL_LOG_SCALE] = (
                    s[0]["log_scale"][j] if "log_scale" in s[0] else False
                )
            except:
                print(sys.exc_info())
            try:
                self.grid_graph.table.data[i][self.grid_graph.table.COL_LABEL] = (
                    s[0]["labels"][j]
                    if "labels" in s[0] and s[0]["labels"] is not None
                    else None
                )
            except:
                print(sys.exc_info())
        ...
        try:
            self.checkBox_x_grid.SetValue(
                s[0]["grids"][0] if "grids" in s[0] else False
            )
        except:
            print(sys.exc_info())
        try:
            self.checkBox_y_grid.SetValue(
                s[0]["grids"][1] if "grids" in s[0] else False
            )
        except:
            print(sys.exc_info())
        try:
            self.comboBox_aspect.SetValue(
                (
                    "auto"
                    if s[0]["aspect"] is None
                    else (
                        s[0]["aspect"]
                        if s[0]["aspect"] in ("auto", "equal")
                        else "{:g}".format(s[0]["aspect"])
                    )
                )
                if "aspect" in s[0]
                else "auto"
        ...
        try:
            if "legend_location" in s[0] and s[0]["legend_location"] is not None:
                self.textCtrl_legend_left.SetValue(
                    "{:g}".format(s[0]["legend_location"][0])
                    if s[0]["legend_location"][0] is not None
                    else ""
                )
                self.textCtrl_legend_top.SetValue(
                    "{:g}".format(s[0]["legend_location"][1])
                    if s[0]["legend_location"][1] is not None
                    else ""
                )
            else:
                self.textCtrl_legend_left.SetValue("")
        ...
        try:
            if "graph_edges" in s[0] and s[0]["graph_edges"] is not None:
                ...
                if b is not None and t is not None and b > t:
                    b, t = t, b
                self.textCtrl_graph_left.SetValue(
                    "{:g}".format(l) if l is not None else ""
                )
                self.textCtrl_graph_bottom.SetValue(
                    "{:g}".format(b) if b is not None else ""
                )
                self.textCtrl_graph_right.SetValue(
                    "{:g}".format(r) if r is not None else ""
                )
                self.textCtrl_graph_top.SetValue(
                    "{:g}".format(t) if t is not None else ""
                )
            else:
                self.textCtrl_graph_left.SetValue("")
                self.textCtrl_graph_bottom.SetValue("")
        ...
        try:
            ...
            if "texts" in s[0] and s[0]["texts"] is not None:
                for i in s[0]["texts"]:
                    self.grid_text.table.data.append(
                        [i[0], i[1], i[2], i[3][0], i[4], i[5], i[6]]
                    )
            ...
            print(sys.exc_info())
        for i, j in (
            ("arrows", self.grid_arrow),
            ("ellipses", self.grid_ellipse),
            ("polygons", self.grid_polygon),
            ("naca4", self.grid_naca4),
        ):
            try:
        ...
        try:
            ...
            for i in range(len(self.grid_param_dict.table.data), 3):
                self.grid_param_dict.table.data.append(
                    self.grid_param_dict.table.new_data[:]
                )
        ...
        n = 0
        for x in s[1:]:
            if x["type"] == "scatter" and (
                "file_name" in x
                and x["file_name"] is not None
                or "equation" in x
                and x["equation"] is not None
            ):
                if n == n_scatter:
                    continue
                try:
                ...
                try:
                    self.filePickers_scatter[n].SetPath(
                        correct_file_name_in_unicode(
                            os.path.join(dir_name, x["file_name"])
                        )
                    )
                except:
                    try:
                        ...
                        print(sys.exc_info())
                for i, j in (
                    (self.textCtrls_scatter_column_x[n], 0),
                    (self.textCtrls_scatter_column_y[n], 1),
                    ...
                    try:
                        if type(x["columns"][j]) is str:
                            ...
                            i.SetValue("{:d}".format(x["columns"][j]))
                    except:
                        i.SetValue("")
                self.textCtrls_scatter_err_column_xn[n].SetValue("")
                self.textCtrls_scatter_err_column_xp[n].SetValue("")
                self.textCtrls_scatter_err_column_yn[n].SetValue("")
                self.textCtrls_scatter_err_column_yp[n].SetValue("")
                    ...
                    for i, j in enumerate(x["err_columns"]):
                        try:
                            if type(j) is tuple:
                                j = [
                                    k
                                    for k in sorted(set(j), key=j.index)
                                    if k is not None
                                ]
                                if len(j) == 0:
                                    pass
                                    ...
                                    if type(j[0]) is str:
                                        (
                                            self.textCtrls_scatter_err_column_xn[n]
                                            if i == 0
                                            else self.textCtrls_scatter_err_column_yn[n]
                                        ).SetValue(
                                            "'"
                                            + (
                                                j[0].decode("UTF-8")
                                    ...
                                    if type(j[1]) is str:
                                        (
                                            self.textCtrls_scatter_err_column_xp[n]
                                            if i == 0
                                            else self.textCtrls_scatter_err_column_yp[n]
                                        ).SetValue(
                                            "'"
                                            + (
                                                j[1].decode("UTF-8")
                            ...
                            print(sys.exc_info())
                try:
                    self.textCtrls_scatter_every[n].SetValue("{:d}".format(x["every"]))
                except:
                    self.textCtrls_scatter_every[n].SetValue("1")
                try:
                    self.textCtrls_scatter_label[n].SetValue(
                        x["label_in_legend"]
                        if "label_in_legend" in x and x["label_in_legend"] is not None
                        else ""
                    )
                except:
                    print(sys.exc_info())
                try:
                    self.choices_scatter_marker[n].SetSelection(
                        markers.index(x["marker"])
                    )
                except:
                    print(sys.exc_info())
                try:
                    self.textCtrls_scatter_marker_size[n].SetValue(
                        "{:g}".format(x["marker_size"])
                        if "marker_size" in x and x["marker_size"] is not None
                        else ""
                    )
                except:
                    print(sys.exc_info())
                ...
                for i, j in (
                    (self.colourPickers_scatter_marker[n], "marker_color"),
                    ...
                    try:
                        if j in x:
                            if x[j] is None:
                                i.SetColour(
                                    wx.WHITE if j == "marker_color" else wx.BLACK
                                )
                            elif (
                                sys.version_info.major <= 2
                                and type(x[j]) is unicode
                                or type(x[j]) is str
                            ):
                                if j == "marker_edge_color" and x[j] == "none":
                                    self.checkBoxes_scatter_marker_edge[n].SetValue(
                                        False
                                    )
                                else:
                                    i.SetColour(x[j])
                                ...
                                i.SetColour(
                                    wx.Colour(
                                        min(255, int(255 * x[j][0])),
                                        min(255, int(255 * x[j][1])),
                                        min(255, int(255 * x[j][2])),
                                    )
                                )
                    except:
                        print(sys.exc_info())
                try:
                    self.choices_scatter_line_style[n].SetSelection(
                        line_styles.index(x["line_style"])
                    )
                except:
                    print(sys.exc_info())
                try:
                    if "show_z_by" not in x or x["show_z_by"] is None:
                        self.textCtrls_scatter_column_z[n].SetValue("")
                    else:
                        self.choices_show_z_by[n].SetSelection(
                            by_what_show_z.index(x["show_z_by"])
                        )
                except:
                    print(sys.exc_info())
                try:
                    self.textCtrls_scatter_marker_size_ratio[n].SetValue(
                        "{:g}".format(x["marker_size_ratio"])
                        if "marker_size_ratio" in x
                        and x["marker_size_ratio"] is not None
                        else ""
                    )
                except:
                    print(sys.exc_info())
                ...
                try:
                    ...
                    print(sys.exc_info())
                for i, j in (
                    (self.textCtrl_vector_column_x, 0),
                    (self.textCtrl_vector_column_y, 1),
                    (self.textCtrl_vector_column_u, 2),
                    (self.textCtrl_vector_column_v, 3),
                ):
                    try:
                        if type(x["columns"][j]) is str:
                            i.SetValue(
                        ...
                        i.SetValue("")
                try:
                    self.textCtrl_vector_every.SetValue("{:d}".format(x["every"]))
                except:
                    self.textCtrl_vector_every.SetValue("1")
                try:
                    if x["color"] == "len":
                        ...
                        if x["color"] is None:
                            ...
                            sys.version_info.major <= 2
                            and type(x["color"]) is unicode
                            or type(x["color"]) is str
                        ):
                            self.colourPicker_vector.SetColour(x["color"])
                        else:  # tuple, each element is in a range between 0 and 1
                            self.colourPicker_vector.SetColour(
                                wx.Colour(
                                    min(255, int(255 * x["color"][0])),
                                    min(255, int(255 * x["color"][1])),
                    ...
                    print(sys.exc_info())
                try:
                    self.textCtrl_vector_scale.SetValue(
                        "{:g}".format(x["scale"])
                        if "scale" in x and x["scale"] is not None
                        else ""
                    )
                except:
                    print(sys.exc_info())
                for i, j in (
                    (self.textCtrl_vector_legend_x, 0),
                    (self.textCtrl_vector_legend_y, 1),
                    (self.textCtrl_vector_legend_u, 2),
                ):
                    try:
                        i.SetValue(
                            "{:g}".format(x["legend"][j])
                            if "legend" in x and x["legend"] is not None
                            else ""
                        )
                    except:
                        print(sys.exc_info())
                try:
                    self.textCtrl_vector_legend_label.SetValue(
                        x["legend"][3]
                        if "legend" in x and x["legend"] is not None
                        else ""
                    )
                ...
                try:
                    ...
                    print(sys.exc_info())
                try:
                    self.filePicker_contour.SetPath(
                        correct_file_name_in_unicode(
                            os.path.join(dir_name, x["file_name"])
                        )
                    )
                    ...
                    print(sys.exc_info())
                self.checkBox_contour_grid_pattern.SetValue(
                    (True if x["structured_grid"] is not None else False)
                    if "structured_grid" in x
                    else False
                )
                self.checkBox_contour_show_triangle.SetValue(
                    x["show_triangle"] if "show_triangle" in x else False
                )
                for i, j in (
                    (self.textCtrl_contour_column_x, 0),
                    (self.textCtrl_contour_column_y, 1),
                    ...
                    try:
                        ...
                        self.i.SetValue("")
                try:
                    self.textCtrl_contour_every.SetValue("{:d}".format(x["every"]))
                except:
                    self.textCtrl_contour_every.SetValue("1")
                try:
                    self.choice_contour_paint.SetSelection(
                        paint_styles.index(x["paint"])
                    )
                except:
                    print(sys.exc_info())
                self.checkBox_contour_smooth_paint.SetValue(
                    x["smooth_paint"] if "smooth_paint" in x else True
                )
    ...
    def menuItem_openOnMenuSelection(self, event):
        with wx.FileDialog(
            self, _("設定ファイルを開く"), style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
        ) as fd:
    ...
    def MakeSettingStrings(self, path):
        ...
        try:
            ...
            if v == "":
                s += "    'png_file_name': None,\n"
            else:
                if not v.endswith(".png"):
                    v += ".png"
                self.filePicker_png.SetPath(v)
                v = correct_file_name_in_unicode(
                    os.path.relpath(v, start=os.path.dirname(path))
                )
                s += "    'png_file_name': '{}',\n".format(
            ...
            try:
                s += "    'fig_size': ({:g}, {:g}),\n".format(
                    float(self.textCtrl_fig_size_w.GetValue()),
                    float(self.textCtrl_fig_size_h.GetValue()),
                )
            except:
                s += "    'fig_size': None,\n"
            for i, j in (
                ...
                if v == "":
                    ...
                    s += "    '{}': '{}',\n".format(
                        j, v.replace("\\", r"\\").replace("'", r"\'")
                    )
            s += "    'ranges': ("
            for i, j, k in (
                (
                    self.grid_graph.table.ROW_X,
                    self.grid_graph.table.COL_FROM,
                    self.grid_graph.table.COL_TO,
                ),
                (
                    self.grid_graph.table.ROW_Y,
                    self.grid_graph.table.COL_FROM,
                    self.grid_graph.table.COL_TO,
                ),
                (
                    self.grid_graph.table.ROW_Z,
                    self.grid_graph.table.COL_FROM,
                    self.grid_graph.table.COL_TO,
                ),
            ):
                try:
                    s += "({:g}, ".format(self.grid_graph.table.data[i][j])
                    try:
                        s += "{:g}), ".format(self.grid_graph.table.data[i][k])
                    except:
                        s += "None), "
                except:
                    try:
                        s += "(None, {:g}), ".format(self.grid_graph.table.data[i][k])
                    except:
                        s += "None, "
            s = s[:-2] + "),\n"
            s += "    'ticks': ("
            ...
            s += (
                ...
                + str(
                    (
                        self.grid_graph.table.data[self.grid_graph.table.ROW_X][
                            self.grid_graph.table.COL_LOG_SCALE
                        ],
                        self.grid_graph.table.data[self.grid_graph.table.ROW_Y][
                            self.grid_graph.table.COL_LOG_SCALE
                        ],
                        self.grid_graph.table.data[self.grid_graph.table.ROW_Z][
                            self.grid_graph.table.COL_LOG_SCALE
                        ],
                    )
                )
            ...
            s = s[:-2] + "),\n"
            s += ("    'grids': ({}, {}),\n").format(
                str(self.checkBox_x_grid.GetValue()),
                str(self.checkBox_y_grid.GetValue()),
            )
            v = self.comboBox_aspect.GetValue()  # unicode
            if v == "":
                s += "    'aspect': 'auto',\n"
            elif v in ("auto", "equal"):
                s += "    'aspect': '{}',\n".format(v)
            else:
                try:
                    s += "    'aspect': {:g},\n".format(float(v))
            ...
            if x == "" and y == "":
                s += "    'legend_location': None,\n"
            else:
                s += "    'legend_location': ({}, {}),\n".format(
                    "None" if x == "" else "{:g}".format(float(x)),
                    "None" if y == "" else "{:g}".format(float(y)),
                )
            ...
            if l == "" and b == "" and r == "" and t == "":
                ...
                if l != "" and r != "" and float(l) > float(r):
                    l, r = r, l
                if b != "" and t != "" and float(b) > float(t):
                    b, t = t, b
                l = "None" if l == "" else "{:g}".format(float(l))
                b = "None" if b == "" else "{:g}".format(float(b))
                r = "None" if r == "" else "{:g}".format(float(r))
                t = "None" if t == "" else "{:g}".format(float(t))
                s += "    'graph_edges': ({}, {}, {}, {}),\n".format(l, b, r, t)
            v = []
            ...
            if len(v) == 0:
                ...
                s += "    'ellipses': ("
                for i in v:
                    s += "('{}', {:g}, {:g}, {:g}, {:g}, {:g}, {:g}, ".format(
                        *i[: self.grid_ellipse.table.COL_FILL]
                    )
            ...
            if len(v) == 0:
                ...
                for i in v:
                    s += "('{}', '{}', {:g}, {:g}, {:g}, {:g}, {:g}, ".format(
                        *i[: self.grid_naca4.table.COL_FILL]
                    )
                    s += str(i[self.grid_naca4.table.COL_FILL])
                    s += ", {}), ".format(i[self.grid_naca4.table.COL_ZORDER])
                s = s[:-1] + "),\n"
            v = []
            ...
            for n in range(n_scatter):
                ...
                s += "{\n    'type': 'scatter',\n"
                s += "    'zorder': {},\n".format(
                    self.spinCtrls_zorder_scatter[n].GetValue()
                )
                ...
                if file_plot:
                    ...
                    s += "    'columns': ("
                    for i in (
                        self.textCtrls_scatter_column_x[n].GetValue(),
                        self.textCtrls_scatter_column_y[n].GetValue(),
                        self.textCtrls_scatter_column_z[n].GetValue(),
                    ):  # unicode
                        if i == "":
                            s += "None, "
                        elif (
                            ...
                            try:
                                ...
                                s += "'{}', ".format(i.strip("'"))
                    s = s[: -8 if s.endswith(", None, ") else -2] + "),\n"
                    s += "    'err_columns': ("
                    for v in (
                        (
                            self.textCtrls_scatter_err_column_xn[n].GetValue(),
                            self.textCtrls_scatter_err_column_xp[n].GetValue(),
                        ),
                        (
                            self.textCtrls_scatter_err_column_yn[n].GetValue(),
                            self.textCtrls_scatter_err_column_yp[n].GetValue(),
                        ),
                    ):  # unicode
                        v = [i for i in sorted(set(v), key=v.index) if i != ""]
                        if len(v) == 0:
                            s += "None, "
                        elif len(v) == 1:
                            if (
                    ...
                    try:
                        s += "    'every': {},\n".format(
                            int(self.textCtrls_scatter_every[n].GetValue())
                        )
                    except:
                        s += "    'every': 1,\n"
                else:  # not file_plot
                ...
                try:
                    s += "    'marker_size': {:g},\n".format(
                        float(self.textCtrls_scatter_marker_size[n].GetValue())
                    )
                except:
                    s += "    'marker_size': None,\n"
                v = self.colourPickers_scatter_marker[n].GetColour()
                s += "    'marker_color': ({:g}, {:g}, {:g}),\n".format(
                    v.Red() / 255.0, v.Green() / 255.0, v.Blue() / 255.0
                )
                if self.checkBoxes_scatter_marker_edge[n].GetValue():
                    v = self.colourPickers_scatter_marker_edge[n].GetColour()
                    s += "    'marker_edge_color': ({:g}, {:g}, {:g}),\n".format(
                        v.Red() / 255.0, v.Green() / 255.0, v.Blue() / 255.0
                    )
                else:
                    s += "    'marker_edge_color': 'none',\n"  # not 'None', but 'none'
                ...
                if self.textCtrls_scatter_column_z[n].GetValue() == "":
                    s += "    'show_z_by': None,\n"
                else:
                    s += "    'show_z_by': '{}',\n".format(
                        by_what_show_z[self.choices_show_z_by[n].GetSelection()]
                    )
                try:
                    s += "    'marker_size_ratio': {:g},\n".format(
                        float(self.textCtrls_scatter_marker_size_ratio[n].GetValue())
                    )
                except:
                    s += "    'marker_size_ratio': None,\n"
                v = self.textCtrls_scatter_label[n].GetValue()  # unicode
                if v == "":
                    s += "    'label_in_legend': None,\n"
                else:
                    s += "    'label_in_legend': '{}',\n".format(
                        v.replace("\\", r"\\").replace("'", r"\'")
                    )
            ...
            if fn != "":
                s += "{\n    'type': 'vector',\n"
                s += "    'zorder': {},\n".format(
                    self.spinCtrl_zorder_vector.GetValue()
                )
                ...
                if file_plot:
                    ...
                    s += "    'columns': ("
                    for i in (
                        self.textCtrl_vector_column_x.GetValue(),
                        self.textCtrl_vector_column_y.GetValue(),
                        self.textCtrl_vector_column_u.GetValue(),
                        self.textCtrl_vector_column_v.GetValue(),
                    ):  # unicode
                        if i == "":
                            s += "None, "
                        elif (
                    ...
                    try:
                        s += "    'every': {},\n".format(
                            int(self.textCtrl_vector_every.GetValue())
                        )
                    except:
                        s += "    'every': 1,\n"
                else:  # not file_plot
                ...
                try:
                    s += "    'scale': {:g},\n".format(
                        float(self.textCtrl_vector_scale.GetValue())
                    )
                except:
                    s += "    'scale': 1.0,\n"
                try:
            ...
            if fn != "":
                s += "{\n    'type': 'contour',\n"
                s += "    'zorder': {},\n".format(
                    self.spinCtrl_zorder_contour.GetValue()
                )
                ...
                s += (
                    "    'grid_pattern': "
                    + str(self.checkBox_contour_grid_pattern.GetValue())
                    + ",\n"
                )
                s += (
                    "    'show_triangle': "
                    + str(self.checkBox_contour_show_triangle.GetValue())
                    + ",\n"
                )
                if file_plot:
                    s += "    'columns': ("
                    for i in (
                        self.textCtrl_contour_column_x.GetValue(),
                        self.textCtrl_contour_column_y.GetValue(),
                        self.textCtrl_contour_column_z.GetValue(),
                    ):  # unicode
                        if i == "":
                            s += "None, "
                        elif (
                    ...
                    try:
                        s += "    'every': {},\n".format(
                            int(self.textCtrl_contour_every.GetValue())
                        )
                    except:
                        s += "    'every': 1,\n"
                ...
                s += (
                    "    'smooth_paint': "
                    + str(self.checkBox_contour_smooth_paint.GetValue())
                    + ",\n"
                )
                s += "},\n"
        except:
    ...
    def menuItem_save_asOnMenuSelection(self, event):
        with wx.FileDialog(
            ...
            fd.SetDirectory(self.cwd)
            fd.SetFilename("settings_matplotlib.txt")
            if fd.ShowModal() == wx.ID_CANCEL:
                return
    ...
    def menuItem_save_as_scriptOnMenuSelection(self, event):
        with wx.FileDialog(
            self,
            _("pythonスクリプト形式で保存"),
            wildcard="python script files (*.py)|*.py",
            style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        ) as fd:
            fd.SetDirectory(self.cwd)
            fd.SetFilename("script_matplotlib.py")
            if fd.ShowModal() == wx.ID_CANCEL:
                return
    ...
    def menuItem_updateOnMenuSelection(self, event):
        ...
        s = get_file_from_github_public(
            user="gitwamoto",
            repository="matplotlibwx",
            branch="main",
            file_path="matplotlibwx.py",
        )
        if s is None:
            with wx.MessageDialog(
                self,
                _("GitHubに接続できませんでした．後でやり直して下さい．"),
                _("接続エラー"),
                style=wx.ICON_ERROR,
            ) as md:
                md.ShowModal()
        ...
        if r is not None and time_str_a_is_newer_than_b(a=r[1].decode(s[1]), b=version):
            p = correct_file_name_in_unicode(
                os.path.realpath(decode_if_necessary(__file__))
            )
            with open(p, "wb") as f:
                f.write(s[0])
            ...
            s = get_file_from_github_public(
                ...
                repository="matplotlibwx",
                branch="main",
                file_path="locale/en/LC_MESSAGES/messages.mo",
            )
            if s is not None:
            ...
            s = get_file_from_github_public(
                ...
                repository="matplotlibwx",
                branch="main",
                file_path="locale/en/LC_MESSAGES/messages.po",
            )
            if s is not None:
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] == "-h":
            print(
                "Usage: %s {<setting_file_name> | -t [<template_file_name>]}"
            ...
            print("Template file '%s' has be made." % t)
            with open("bbb_b.txt", "wb") as f:
                f.write(b)  # mac + python 2 -> success, mac + python 3 -> ?
        else:
            plot(
                load_plot_settings(os.path.abspath(decode_if_necessary(sys.argv[1])))[
                    0
                ],
                show=True,
            )
